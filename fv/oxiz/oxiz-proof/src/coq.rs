...
mod tests {
    ...
    fn test_escape_string() {
        ...
        // pass through unchanged: Coq string literals accept raw UTF-8, and
        // only `"`/`\` are special to them.
        assert_eq!(CoqExporter::escape_string("caf\u{e9}"), "caf\u{e9}");
        assert_eq!(CoqExporter::escape_string("a\u{0}b"), "a\u{0}b");
    }

    /// Confirmation (not a fix): a `"` in a conclusion could break the
    /// `"..."` Coq string literal `conclusion_to_prop` wraps every
    ...
    fn test_export_proof_conclusion_with_quote_keeps_string_literal_balanced() {
        ...
        assert!(
            coq_code.contains("\\\"quote\\\""),
            "the quote must be escaped, not raw, in: {coq_code}"
        );
        for line in coq_code.lines().filter(|l| l.contains("Prop_of_string \"")) {
            let after = line
                .split_once("Prop_of_string \"")
            ...
            assert!(
                terminated,
                "the Prop_of_string literal never reaches an unescaped closing quote in: {line}"
            );
        }
    }
