...
pub enum SimpDirection {
    /// Rewrite left-to-right (standard).
    Forward,
...
pub struct SimpResult {
    ...
    pub expr: Expr,
    /// Names of lemmas that were applied (in order).
...
impl SimpResult {
    ...
    pub fn simplified(expr: Expr, applied: Vec<Name>) -> Self {
...
pub struct SimpLemmaSet {
    lemmas: Vec<SimpLemma>,
}
impl SimpLemmaSet {
    /// Create an empty lemma set.
    pub fn new() -> Self {
        Self { lemmas: Vec::new() }
    }
    /// Add a lemma.
    ...
    /// Apply all lemmas exhaustively (until no changes).
    pub fn apply_all(&self, expr: &Expr) -> Expr {
        let mut current = expr.clone();
        loop {
            let (new_expr, changed) = self.apply_once(&current);
            if !changed {
                return current;
            }
            current = new_expr;
    ...
    /// Iter over lemmas.
    pub fn iter(&self) -> impl Iterator<Item = &SimpLemma> {
        self.lemmas.iter()
    }
    /// Look up a lemma by name.
    pub fn find(&self, name: &Name) -> Option<&SimpLemma> {
...
impl SimpLemma {
    /// Create a forward simp lemma.
    pub fn forward(name: Name, lhs: Expr, rhs: Expr) -> Self {
        Self {
            name,
            lhs,
            rhs,
    ...
    /// Create a backward simp lemma.
    pub fn backward(name: Name, lhs: Expr, rhs: Expr) -> Self {
        Self {
            name,
            lhs,
            rhs,
