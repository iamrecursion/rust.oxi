...
/// An ordered sequence of axioms with dependency tracking.
///
/// Axioms are stored in declaration order and can be queried by index.
#[allow(dead_code)]
pub struct AxiomSequence {
    entries: Vec<(Name, AxiomCategory)>,
}
#[allow(dead_code)]
impl AxiomSequence {
    ...
    /// Append an axiom to the sequence.
    pub fn push(&mut self, name: Name) {
        let cat = classify_axiom_category(&name);
        self.entries.push((name, cat));
    }
    /// Number of axioms in the sequence.
    ...
    /// Get the axiom at index `i`.
    pub fn get(&self, i: usize) -> Option<&(Name, AxiomCategory)> {
        self.entries.get(i)
    }
    /// Return all classical axioms in the sequence.
    pub fn classical_axioms(&self) -> Vec<&Name> {
        self.entries
            .iter()
            .filter(|(_, c)| c == &AxiomCategory::Classical)
            .map(|(n, _)| n)
            .collect()
    }
    /// Return all user-defined axioms in the sequence.
    pub fn user_axioms(&self) -> Vec<&Name> {
        self.entries
            .iter()
            .filter(|(_, c)| c == &AxiomCategory::UserDefined)
            .map(|(n, _)| n)
            .collect()
    }
    /// Check if the sequence contains an axiom with the given name.
    pub fn contains(&self, name: &Name) -> bool {
        self.entries.iter().any(|(n, _)| n == name)
    }
    /// Remove all classical axioms from the sequence.
    pub fn remove_classical(&mut self) {
        self.entries.retain(|(_, c)| c != &AxiomCategory::Classical);
    }
    /// Clear all entries.
...
/// A record of how an axiom is used in a proof context.
#[allow(dead_code)]
pub struct AxiomUsageRecord {
    /// The axiom name.
    pub name: Name,
    /// Number of times this axiom is directly referenced.
    pub direct_uses: u32,
    /// Number of definitions that transitively depend on this axiom.
    pub transitive_deps: u32,
    /// Whether this axiom appears in a Prop-valued context.
    pub in_prop_context: bool,
}
#[allow(dead_code)]
impl AxiomUsageRecord {
    ...
    pub fn new(name: Name) -> Self {
        AxiomUsageRecord {
            ...
            direct_uses: 0,
            transitive_deps: 0,
            in_prop_context: false,
        }
    }
    /// Increment the direct-use count.
    pub fn record_direct_use(&mut self) {
        self.direct_uses += 1;
    }
    /// Increment the transitive-dependency count.
    pub fn record_transitive_dep(&mut self) {
        self.transitive_deps += 1;
    }
}
...
pub enum AxiomSafety {
    /// Safe axiom (known to be consistent)
    Safe,
    /// Potentially unsafe (needs review, e.g., classical axioms)
    Questionable,
    /// Known unsafe (could break consistency if misused)
    Unsafe,
}
...
/// Categories of axioms for safety analysis.
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum AxiomCategory {
    /// Classical axioms (e.g., excluded middle, choice).
    Classical,
    /// Definitional axioms (safe, by construction).
    Definitional,
    /// External axioms (from FFI or plugins).
    External,
    /// User-defined axioms (declared with `axiom`).
    UserDefined,
    /// Propositional axioms (safe in most foundations).
    Propositional,
}
/// A write-once cell.
#[allow(dead_code)]
...
/// A safety report for an entire environment.
#[allow(dead_code)]
pub struct AxiomSafetyReport {
    /// Number of axioms declared.
    pub total_axioms: usize,
    /// Number of axioms classified as classical.
    pub classical_count: usize,
    /// Number of axioms classified as external.
    pub external_count: usize,
    /// Whether the environment is classically consistent.
    pub classically_consistent: bool,
    /// Warnings accumulated during analysis.
    pub warnings: Vec<String>,
}
#[allow(dead_code)]
impl AxiomSafetyReport {
    ...
    pub fn empty() -> Self {
        AxiomSafetyReport {
            ...
            classical_count: 0,
            external_count: 0,
            classically_consistent: true,
            warnings: Vec::new(),
        }
    }
    /// Return true iff the environment has no axioms.
...
pub struct AxiomValidator {
    /// Declared axioms
    axioms: HashSet<Name>,
    /// Unsafe axioms (those that could break consistency)
    unsafe_axioms: HashSet<Name>,
    /// Known safe axioms
    pub(super) safe_axioms: HashSet<Name>,
    /// Classical axioms
    classical_axioms: HashSet<Name>,
}
impl AxiomValidator {
    /// Create a new axiom validator.
    pub fn new() -> Self {
        let mut classical = HashSet::new();
        classical.insert(Name::str("Classical.choice"));
        classical.insert(Name::str("Classical.em"));
        classical.insert(Name::str("Classical.byContradiction"));
        let mut safe = HashSet::new();
        safe.insert(Name::str("propext"));
        safe.insert(Name::str("Quot.mk"));
        safe.insert(Name::str("Quot.lift"));
        safe.insert(Name::str("Quot.ind"));
        safe.insert(Name::str("Quot.sound"));
        Self {
            axioms: HashSet::new(),
            unsafe_axioms: HashSet::new(),
            safe_axioms: safe,
            classical_axioms: classical,
        }
    }
    /// Register an axiom.
    pub fn register(&mut self, name: Name) {
        self.axioms.insert(name);
    }
    /// Mark an axiom as unsafe.
    pub fn mark_unsafe(&mut self, name: Name) {
        self.unsafe_axioms.insert(name);
    }
    /// Mark an axiom as safe.
    pub fn mark_safe(&mut self, name: Name) {
        self.safe_axioms.insert(name);
    }
    /// Check if a name is a registered axiom.
    pub fn is_axiom(&self, name: &Name) -> bool {
        self.axioms.contains(name)
    }
    /// Check if an axiom is marked as unsafe.
    pub fn is_unsafe(&self, name: &Name) -> bool {
        self.unsafe_axioms.contains(name)
    }
    /// Check if an axiom is classical.
    pub fn is_classical(&self, name: &Name) -> bool {
        self.classical_axioms.contains(name)
    }
    /// Get all registered axioms.
    pub fn all_axioms(&self) -> Vec<&Name> {
        self.axioms.iter().collect()
    }
    /// Get all unsafe axioms.
    pub fn unsafe_axioms(&self) -> Vec<&Name> {
        self.unsafe_axioms.iter().collect()
    }
    /// Classify an axiom's safety.
    pub fn classify(&self, name: &Name) -> AxiomSafety {
        if self.unsafe_axioms.contains(name) {
            AxiomSafety::Unsafe
        } else if self.classical_axioms.contains(name) {
            AxiomSafety::Questionable
        } else if self.safe_axioms.contains(name) {
            AxiomSafety::Safe
        } else {
            AxiomSafety::Questionable
        }
...
/// An allowlist of permitted axioms.
#[allow(dead_code)]
pub struct AxiomAllowlist {
    /// Allowed axiom names.
    allowed: HashSet<Name>,
}
#[allow(dead_code)]
impl AxiomAllowlist {
    /// Create an empty allowlist (nothing allowed).
    pub fn empty() -> Self {
        AxiomAllowlist {
            allowed: HashSet::new(),
        }
    }
    /// Create an allowlist with the standard classical axioms permitted.
    pub fn classical() -> Self {
        let mut al = Self::empty();
        al.allowed.insert(Name::str("Classical.em"));
        al.allowed.insert(Name::str("Classical.choice"));
        al.allowed.insert(Name::str("propext"));
        al.allowed.insert(Name::str("funext"));
        al
    }
    /// Add an axiom to the allowlist.
    pub fn allow(&mut self, name: Name) {
        self.allowed.insert(name);
    }
    /// Check whether a given axiom is allowed.
    pub fn is_allowed(&self, name: &Name) -> bool {
        self.allowed.contains(name)
    }
    /// Verify that all axioms in the environment are in the allowlist.
    pub fn check_env(&self, env: &Environment) -> Result<(), Vec<Name>> {
        let disallowed: Vec<Name> = extract_axioms(env)
            .into_iter()
