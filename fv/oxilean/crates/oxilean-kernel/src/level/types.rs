...
pub enum LevelConstraint {
    /// l1 ≤ l2
    Leq(Level, Level),
    /// l1 = l2 (syntactic equality after normalization)
    Eq(Level, Level),
}
impl LevelConstraint {
    /// Check whether this constraint is satisfied.
    ///
    /// Uses `is_leq` / `is_equivalent` from this module.
    #[allow(dead_code)]
    pub fn is_satisfied(&self) -> bool {
        match self {
            LevelConstraint::Leq(l1, l2) => is_leq(l1, l2),
            LevelConstraint::Eq(l1, l2) => is_equivalent(l1, l2),
        }
    }
    /// Return both levels involved in this constraint.
    #[allow(dead_code)]
    pub fn levels(&self) -> (&Level, &Level) {
        match self {
            LevelConstraint::Leq(l1, l2) | LevelConstraint::Eq(l1, l2) => (l1, l2),
        }
    }
}
...
pub struct LevelMVarId(pub u64);
