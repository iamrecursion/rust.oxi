...
pub struct WhnfStats {
    ...
    pub beta_steps: u64,
    /// Number of delta (definition unfolding) steps.
    pub delta_steps: u64,
    /// Number of zeta (let binding) steps.
    pub zeta_steps: u64,
    /// Number of iota (recursor) steps.
    pub iota_steps: u64,
...
impl WhnfStats {
    ...
    /// Total number of reduction steps performed.
    pub fn total_steps(&self) -> u64 {
        self.beta_steps + self.delta_steps + self.zeta_steps + self.iota_steps
    }
...
pub struct ReductionBudget {
    /// Remaining number of steps allowed.
    pub remaining: u64,
}
impl ReductionBudget {
    /// Create a new budget allowing n steps.
    ...
    /// Consume one step. Returns false if the budget is exhausted.
...
impl WhnfDepthBudget {
    ...
    /// Create an unlimited budget (u32::MAX).
    pub fn unlimited() -> Self {
        Self {
            budget: u32::MAX,
            remaining: u32::MAX,
    ...
    /// Consume one step. Returns `true` if budget still remains.
    pub fn consume(&mut self) -> bool {
        if self.remaining > 0 {
            self.remaining -= 1;
            true
...
/// The head of an application spine after WHNF reduction.
...
impl WhnfHead {
    ...
    pub fn to_expr(self) -> Expr {
        match self {
            WhnfHead::Sort(l) => Expr::Sort(l),
            WhnfHead::BVar(i) => Expr::BVar(i),
            WhnfHead::FVar(id) => Expr::FVar(id),
            WhnfHead::Const(n, ls) => Expr::Const(n, ls),
            WhnfHead::Lam(bi, n, ty, body) => Expr::Lam(bi, n, ty, body),
            WhnfHead::Pi(bi, n, ty, body) => Expr::Pi(bi, n, ty, body),
            WhnfHead::Lit(l) => Expr::Lit(l),
        }
    }
    /// Return the name if this head is a Const.
    pub fn as_const_name(&self) -> Option<&Name> {
        match self {
            WhnfHead::Const(n, _) => Some(n),
            _ => None,
        }
    }
}
...
///
/// Stores the most recently computed WHNF reductions to avoid
/// redundant work when the same expression appears multiple times.
#[derive(Debug, Default)]
pub struct WhnfCache {
    /// Cached entries: (key, result) in insertion order.
    entries: Vec<(WhnfCacheKey, Expr)>,
...
pub struct WhnfConfig {
    /// Whether to apply beta reductions.
    pub beta: bool,
    /// Whether to apply zeta (let) reductions.
    pub zeta: bool,
    /// Whether to unfold definitions (delta).
    pub delta: bool,
    /// Whether to apply iota (recursor) reductions.
    pub iota: bool,
    /// Maximum reduction steps (0 = unlimited).
    pub max_steps: u64,
}
impl WhnfConfig {
    ...
    /// Disable delta (definition unfolding).
    pub fn no_delta(mut self) -> Self {
        self.delta = false;
        self
    }
    /// Disable beta reduction.
    pub fn no_beta(mut self) -> Self {
        self.beta = false;
        self
    }
    /// Check if at least one reduction is enabled.
    pub fn any_enabled(&self) -> bool {
        self.beta || self.zeta || self.delta || self.iota
    }
}
/// A cache key for memoizing WHNF results.
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
pub struct WhnfCacheKey {
    /// Structural hash of the input expression.
    pub expr_hash: u64,
}
impl WhnfCacheKey {
    /// Compute a cache key from an expression.
