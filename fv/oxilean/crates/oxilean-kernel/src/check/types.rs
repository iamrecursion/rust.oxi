...
///
/// Controls what level of checking is performed and whether certain
/// optional checks are enabled.
#[derive(Debug, Clone)]
pub struct CheckConfig {
    /// Whether to check that the declared type is a sort.
    pub check_type_is_sort: bool,
    /// Whether to check that the value has the declared type.
    pub check_value_type: bool,
    /// Whether to check for free variables in the type and value.
    pub check_no_free_vars: bool,
    /// Maximum depth for type inference (0 = unlimited).
    pub max_depth: u32,
    /// Whether to allow axioms (false = proof-irrelevant mode).
...
impl CheckConfig {
    ...
    /// Create a lenient configuration that skips optional checks.
    pub fn lenient() -> Self {
        CheckConfig {
            check_type_is_sort: true,
            check_value_type: false,
            check_no_free_vars: false,
            max_depth: 0,
            allow_axioms: true,
        }
    }
    /// Create a strict configuration that enables all checks.
    pub fn strict() -> Self {
        CheckConfig {
            check_type_is_sort: true,
...
///
/// NOTE: This struct borrows the environment mutably and removes the added
/// declarations when dropped. This is a lightweight simulation for testing.
pub struct TempEnvScope<'a> {
    env: &'a mut Environment,
    added_names: Vec<crate::Name>,
}
impl<'a> TempEnvScope<'a> {
    ...
    pub fn add(&mut self, decl: Declaration) -> Result<(), KernelError> {
        ...
        check_declaration(self.env, decl)?;
    ...
    /// Return the number of declarations added in this scope.
...
impl DeclKind {
    ...
    /// Returns `true` if this kind represents a proven statement.
    ...
    pub fn is_definition(&self) -> bool {
        matches!(self, DeclKind::Definition | DeclKind::Opaque)
    }
}
...
pub struct CheckStats {
    ...
    /// Number of theorems checked.
    pub num_theorems: usize,
    /// Number of opaque definitions checked.
    pub num_opaques: usize,
    /// Number of failures.
    pub num_failures: usize,
}
impl CheckStats {
    /// Create a new empty statistics record.
    pub fn new() -> Self {
        Self::default()
    }
    /// Total number of declarations processed (including failures).
    pub fn total(&self) -> usize {
        self.num_axioms + self.num_definitions + self.num_theorems + self.num_opaques
    }
...
/// Result of checking a batch of declarations.
#[derive(Debug, Clone)]
pub struct BatchCheckResult {
    /// Number of declarations successfully checked.
    pub num_ok: usize,
    /// Number of declarations that failed.
    pub num_failed: usize,
    /// Errors encountered (name, error description).
    pub errors: Vec<(String, String)>,
}
impl BatchCheckResult {
    /// Returns `true` if all declarations passed.
    pub fn all_ok(&self) -> bool {
        self.num_failed == 0
    }
    /// Returns the total number of declarations processed.
    pub fn total(&self) -> usize {
        self.num_ok + self.num_failed
    }
    /// Returns an iterator over error messages.
    pub fn error_messages(&self) -> impl Iterator<Item = String> + '_ {
        self.errors
            .iter()
...
pub struct DeclSummary {
    ...
    pub kind: DeclKind,
    /// Whether the declaration has a body/proof.
    pub has_body: bool,
...
impl WellFormedResult {
    /// Returns `true` if the result indicates a well-formed declaration.
    pub fn is_ok(&self) -> bool {
        matches!(self, WellFormedResult::Ok)
    }
    /// Returns a human-readable description of the result.
    pub fn description(&self) -> String {
        match self {
            WellFormedResult::Ok => "well-formed".to_string(),
            WellFormedResult::TypeNotSort { description } => {
                format!("type is not a sort: {}", description)
            }
            WellFormedResult::TypeMismatch { description } => {
                format!("type mismatch: {}", description)
            }
            WellFormedResult::FreeVariables { vars } => {
                format!("free variables: {}", vars.join(", "))
            }
            WellFormedResult::DuplicateName { name } => {
                format!("duplicate declaration: {}", name)
            }
            WellFormedResult::UniverseError { description } => {
                format!("universe error: {}", description)
            }
        }
    }
}
...
///
/// Useful for constructing a consistent environment from a sequence of
/// declarations, with error recovery.
#[derive(Default)]
...
impl EnvBuilder {
    ...
    pub fn add(&mut self, decl: Declaration) -> bool {
        ...
        match check_declaration(&mut self.env, decl) {
            Ok(()) => {
                match kind {
                    DeclKind::Axiom => self.stats.num_axioms += 1,
                    DeclKind::Definition => self.stats.num_definitions += 1,
                    DeclKind::Theorem => self.stats.num_theorems += 1,
                    DeclKind::Opaque => self.stats.num_opaques += 1,
                }
                true
            }
            Err(e) => {
    ...
    /// Return a reference to the current errors.
