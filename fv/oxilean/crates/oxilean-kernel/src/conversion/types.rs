...
use super::functions::coercion_head_matches;
...
pub enum ConvResult {
    ...
    NotEqual,
    /// Timeout: conversion check exceeded the step limit.
    /// Timeout: conversion check exceeded the step limit.
    Timeout {
        /// Number of steps performed before timeout.
        steps: usize,
    },
...
impl ConvResult {
    ...
    pub fn is_equal(&self) -> bool {
        matches!(self, ConvResult::Equal)
    }
    /// Check if the result is definitive (not timed out or errored).
    pub fn is_definitive(&self) -> bool {
        matches!(self, ConvResult::Equal | ConvResult::NotEqual)
    }
}
...
impl ConversionChecker {
    /// Create a new conversion checker with default transparency.
    pub fn new() -> Self {
        Self {
            reducer: Reducer::new(),
            max_steps: 10000,
    ...
    pub fn is_convertible(&mut self, e1: &Expr, e2: &Expr) -> bool {
        self.is_convertible_impl(e1, e2, 0)
    }
    ...
    pub fn is_convertible_in_env(&mut self, e1: &Expr, e2: &Expr, env: &Environment) -> bool {
        self.is_convertible_env_impl(e1, e2, env, 0)
    }
    /// Implementation with depth tracking (no environment).
    fn is_convertible_impl(&mut self, e1: &Expr, e2: &Expr, depth: usize) -> bool {
        ...
        let whnf1 = self.reducer.whnf(e1);
        let whnf2 = self.reducer.whnf(e2);
        self.structural_eq(&whnf1, &whnf2, depth)
    }
    /// Implementation with depth tracking (with environment).
    fn is_convertible_env_impl(
        &mut self,
        e1: &Expr,
        e2: &Expr,
        env: &Environment,
        ...
        let whnf1 = self.reducer.whnf_env(e1, env);
        let whnf2 = self.reducer.whnf_env(e2, env);
        self.structural_eq_env(&whnf1, &whnf2, env, depth)
    }
    /// Structural equality check after WHNF.
    fn structural_eq(&mut self, e1: &Expr, e2: &Expr, depth: usize) -> bool {
        if e1 == e2 {
            return true;
        }
        match (e1, e2) {
            ...
            (Expr::Const(n1, ls1), Expr::Const(n2, ls2)) => {
                n1 == n2
                    && ls1.len() == ls2.len()
                    && ls1.iter().zip(ls2.iter()).all(|(a, b)| a == b)
            }
            (Expr::Sort(l1), Expr::Sort(l2)) => crate::level::is_equivalent(l1, l2),
            (Expr::Lit(l1), Expr::Lit(l2)) => l1 == l2,
            (Expr::App(f1, a1), Expr::App(f2, a2)) => {
                self.is_convertible_impl(f1, f2, depth + 1)
                    && self.is_convertible_impl(a1, a2, depth + 1)
            }
            (Expr::Lam(_, _, ty1, body1), Expr::Lam(_, _, ty2, body2)) => {
                self.is_convertible_impl(ty1, ty2, depth + 1)
                    && self.is_convertible_impl(body1, body2, depth + 1)
            }
            ...
            (Expr::Let(_, ty1, val1, body1), Expr::Let(_, ty2, val2, body2)) => {
                self.is_convertible_impl(ty1, ty2, depth + 1)
                    && self.is_convertible_impl(val1, val2, depth + 1)
                    && self.is_convertible_impl(body1, body2, depth + 1)
            }
            (Expr::Proj(n1, i1, e1), Expr::Proj(n2, i2, e2)) => {
                n1 == n2 && i1 == i2 && self.is_convertible_impl(e1, e2, depth + 1)
            }
            _ => false,
        }
    }
    /// Structural equality check with environment.
    fn structural_eq_env(&mut self, e1: &Expr, e2: &Expr, env: &Environment, depth: usize) -> bool {
        if e1 == e2 {
            return true;
        }
        match (e1, e2) {
            ...
            (Expr::Lit(l1), Expr::Lit(l2)) => l1 == l2,
            (Expr::App(f1, a1), Expr::App(f2, a2)) => {
                self.is_convertible_env_impl(f1, f2, env, depth + 1)
                    && self.is_convertible_env_impl(a1, a2, env, depth + 1)
            }
            (Expr::Lam(_, _, ty1, body1), Expr::Lam(_, _, ty2, body2)) => {
                self.is_convertible_env_impl(ty1, ty2, env, depth + 1)
                    && self.is_convertible_env_impl(body1, body2, env, depth + 1)
            }
            (Expr::Pi(_, _, ty1, body1), Expr::Pi(_, _, ty2, body2)) => {
                self.is_convertible_env_impl(ty1, ty2, env, depth + 1)
                    && self.is_convertible_env_impl(body1, body2, env, depth + 1)
            ...
            (Expr::Let(_, ty1, val1, body1), Expr::Let(_, ty2, val2, body2)) => {
                self.is_convertible_env_impl(ty1, ty2, env, depth + 1)
                    && self.is_convertible_env_impl(val1, val2, env, depth + 1)
                    && self.is_convertible_env_impl(body1, body2, env, depth + 1)
            }
            (Expr::Proj(n1, i1, e1), Expr::Proj(n2, i2, e2)) => {
                n1 == n2 && i1 == i2 && self.is_convertible_env_impl(e1, e2, env, depth + 1)
            }
            _ => false,
        }
    }
    ...
    /// Get the maximum number of reduction steps.
...
#[allow(dead_code)]
pub struct Coercion {
    /// Name of the coercion function.
    pub name: crate::Name,
    /// Source type (the type being coerced from).
    pub source: crate::Expr,
    /// Target type (the type being coerced to).
    pub target: crate::Expr,
    /// Priority (higher = preferred when multiple coercions apply).
    pub priority: i32,
}
...
impl CoercionTable {
    ...
    pub fn register(&mut self, coercion: Coercion) {
        let pos = self
            .coercions
            .partition_point(|c| c.priority >= coercion.priority);
        self.coercions.insert(pos, coercion);
    }
    /// Find coercions applicable to a given source type.
    pub fn find(&self, source_head: &crate::Name) -> Vec<&Coercion> {
        self.coercions
            .iter()
            .filter(|c| coercion_head_matches(&c.source, source_head))
            .collect()
    }
