...
use super::functions::alpha_equiv;
...
pub struct AlphaCache {
    known_equiv: Vec<(Expr, Expr)>,
    known_non_equiv: Vec<(Expr, Expr)>,
}
impl AlphaCache {
    ...
    pub fn query(&self, e1: &Expr, e2: &Expr) -> Option<bool> {
        for (a, b) in &self.known_equiv {
            if (a == e1 && b == e2) || (a == e2 && b == e1) {
                return Some(true);
            }
        }
        for (a, b) in &self.known_non_equiv {
            if (a == e1 && b == e2) || (a == e2 && b == e1) {
                return Some(false);
            }
        }
    ...
    pub fn alpha_equiv_cached(&mut self, e1: &Expr, e2: &Expr) -> bool {
        if let Some(result) = self.query(e1, e2) {
            return result;
        }
        let result = alpha_equiv(e1, e2);
        if result {
            self.known_equiv.push((e1.clone(), e2.clone()));
        } else {
            self.known_non_equiv.push((e1.clone(), e2.clone()));
        }
        result
    }
    /// Return the number of cached equivalences.
    pub fn num_equiv(&self) -> usize {
        self.known_equiv.len()
    }
    /// Return the number of cached non-equivalences.
    pub fn num_non_equiv(&self) -> usize {
        self.known_non_equiv.len()
    }
    /// Clear the cache.
    pub fn clear(&mut self) {
        self.known_equiv.clear();
        self.known_non_equiv.clear();
    }
}
