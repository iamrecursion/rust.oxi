...
use super::functions::{instantiate_many, named_subst_apply};

/// A telescope is a sequence of (name, type) binders, where later types
/// may depend on earlier names.
///
/// This is the standard way to represent the argument list of a function
/// type `(x₁ : T₁) → (x₂ : T₂[x₁]) → … → B`.
#[derive(Clone, Debug, Default)]
pub struct Telescope {
    /// Binders in order from outermost to innermost.
...
impl Telescope {
    ...
    /// Add a binder to the end of the telescope.
    pub fn push(&mut self, name: Name, ty: Expr) {
        self.binders.push((name, ty));
    }
    /// Return the length of the telescope.
    pub fn len(&self) -> usize {
        self.binders.len()
    }
    /// Check if the telescope is empty.
    ...
    pub fn to_pi(&self, body: Expr) -> Expr {
        self.binders.iter().rev().fold(body, |acc, (name, ty)| {
            Expr::Pi(
    ...
    /// Reconstruct a Lambda from this telescope and a body.
    ...
    pub fn instantiate(&self, args: &[Expr]) -> Vec<Expr> {
        ...
        for (i, (_, ty)) in self.binders.iter().enumerate() {
            let subst = &args[..i.min(args.len())];
            let inst = instantiate_many(ty, subst);
            result.push(inst);
        }
        result
    }
...
impl NamedSubst {
    ...
    /// Insert a binding.
    pub fn insert(&mut self, name: Name, val: Expr) {
    ...
    ///
    /// Replaces any `Const(name, [])` that appears in the map.
    pub fn apply(&self, expr: &Expr) -> Expr {
        named_subst_apply(expr, self)
    }
}
