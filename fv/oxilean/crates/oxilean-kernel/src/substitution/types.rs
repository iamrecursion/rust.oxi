...
use super::functions::apply_subst;
...
/// A substitution mapping bound variable levels to replacement expressions.
///
/// Allows performing multiple substitutions in a single pass over the expression.
#[derive(Debug, Clone, Default)]
pub struct Substitution {
    /// Mapping from bound variable depth to replacement expression.
    mapping: Vec<Option<Expr>>,
}
impl Substitution {
    ...
    /// Create a substitution that maps BVar(0) to a single expression.
    pub fn single(expr: Expr) -> Self {
        Self {
            mapping: vec![Some(expr)],
        }
    }
    /// Add an entry: map BVar at `level` to `expr`.
    pub fn add(&mut self, level: usize, expr: Expr) {
        if level >= self.mapping.len() {
            self.mapping.resize(level + 1, None);
        }
        self.mapping[level] = Some(expr);
    }
    /// Get the replacement for BVar at `level` (if any).
    pub fn get(&self, level: u32) -> Option<&Expr> {
        self.mapping.get(level as usize).and_then(|o| o.as_ref())
    }
    /// The number of levels this substitution covers.
    pub fn len(&self) -> usize {
        self.mapping.len()
    }
    /// Check if this substitution is empty.
    pub fn is_empty(&self) -> bool {
        self.mapping.iter().all(|o| o.is_none())
    }
    /// Apply this substitution to an expression.
    pub fn apply(&self, expr: &Expr) -> Expr {
        apply_subst(expr, self, 0)
    }
}
