...
pub struct SubstStats {
    /// Number of successful `BVar` hits.
    pub bvar_hits: usize,
    /// Number of `BVar` misses.
    pub bvar_misses: usize,
    /// Number of `FVar` substitutions performed.
    pub fvar_hits: usize,
    /// Number of nodes visited in total.
    pub nodes_visited: usize,
}
impl SubstStats {
    /// Merge another stats record into `self`.
    pub fn merge(&mut self, other: &SubstStats) {
        self.bvar_hits += other.bvar_hits;
        self.bvar_misses += other.bvar_misses;
        self.fvar_hits += other.fvar_hits;
        self.nodes_visited += other.nodes_visited;
    }
    /// Total substitutions performed.
    pub fn total_substs(&self) -> usize {
        self.bvar_hits + self.fvar_hits
    }
}
...
impl Substitution {
    ...
    pub fn insert(&mut self, fvar: FVarId, expr: Expr) {
        if let Some(existing) = self.pairs.iter_mut().find(|(f, _)| *f == fvar) {
            existing.1 = expr;
        } else {
            self.pairs.push((fvar, expr));
        }
    }
    /// Look up the image of a free variable.
    #[allow(dead_code)]
    pub fn get(&self, fvar: FVarId) -> Option<&Expr> {
        self.pairs.iter().find(|(f, _)| *f == fvar).map(|(_, e)| e)
    }
    /// Number of mappings.
    #[allow(dead_code)]
    pub fn len(&self) -> usize {
    ...
    pub fn apply(&self, expr: &Expr) -> Expr {
        ...
        self.apply_inner(expr)
    }
    fn apply_inner(&self, expr: &Expr) -> Expr {
        match expr {
            Expr::FVar(id) => {
                if let Some(replacement) = self.get(*id) {
                    replacement.clone()
                } else {
                    expr.clone()
                }
    ...
    pub fn compose(&self, other: &Substitution) -> Substitution {
        ...
        for (fvar, expr) in &self.pairs {
            result.insert(*fvar, other.apply(expr));
        }
        for (fvar, expr) in &other.pairs {
            if self.get(*fvar).is_none() {
                result.insert(*fvar, expr.clone());
            }
        }
        result
    }
    /// Remove a mapping.
    #[allow(dead_code)]
    pub fn remove(&mut self, fvar: FVarId) {
        self.pairs.retain(|(f, _)| *f != fvar);
    }
