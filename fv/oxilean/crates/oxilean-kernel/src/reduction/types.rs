...
impl RedexKind {
    ...
    pub fn description(&self) -> String {
        match self {
            RedexKind::Beta => "β-redex".to_string(),
            RedexKind::Let => "let-redex".to_string(),
            RedexKind::Proj => "proj-redex".to_string(),
            RedexKind::Delta(name) => format!("δ-redex ({})", name),
        }
    }
}
...
impl ReductionResult {
    /// Get the resulting expression regardless of whether reduction occurred.
    pub fn expr(&self) -> &Expr {
        match self {
            ReductionResult::Reduced(e)
            | ReductionResult::Normal(e)
            | ReductionResult::Aborted(e) => e,
        }
    }
    /// Returns `true` if reduction occurred.
    pub fn was_reduced(&self) -> bool {
        matches!(self, ReductionResult::Reduced(_))
    }
    /// Returns `true` if the expression is in normal form.
    pub fn is_normal(&self) -> bool {
        matches!(self, ReductionResult::Normal(_))
    }
    /// Consume the result and return the expression.
...
pub struct RedexInfo {
    /// The kind of redex.
    pub kind: RedexKind,
    /// Depth in the expression tree where the redex occurs.
    pub depth: usize,
    /// Size of the redex sub-expression.
    pub size: usize,
}
...
pub struct ReductionStep {
    ...
    pub after: Expr,
    /// The rule applied (human-readable description).
    pub rule: String,
}
...
pub struct ReductionMemo {
    entries: std::collections::HashMap<(ReductionStrategy, String), Expr>,
    hits: usize,
    misses: usize,
}
impl ReductionMemo {
    ...
    /// Insert a memo entry.
    pub fn insert(&mut self, strategy: ReductionStrategy, expr: &Expr, result: Expr) {
        let key = (strategy, format!("{:?}", expr));
        self.entries.insert(key, result);
    }
    /// Look up a memo entry.
    pub fn get(&mut self, strategy: ReductionStrategy, expr: &Expr) -> Option<&Expr> {
        let key = (strategy, format!("{:?}", expr));
...
impl HeadForm {
    ...
    pub fn is_reducible(&self) -> bool {
        matches!(self, HeadForm::Let | HeadForm::BetaRedex)
    }
    /// Returns `true` if this is a variable head.
    pub fn is_variable(&self) -> bool {
        matches!(self, HeadForm::BVar(_) | HeadForm::FVar)
    }
    /// Returns `true` if this is a canonical/neutral form.
    pub fn is_neutral(&self) -> bool {
        matches!(
            self,
            HeadForm::BVar(_) | HeadForm::FVar | HeadForm::Const(_)
        )
    }
}
...
pub struct ReductionTrace {
    ...
    /// Whether the trace reached a normal form.
    pub reached_normal: bool,
    /// Whether the trace was truncated (too many steps).
    pub truncated: bool,
}
impl ReductionTrace {
    ...
    /// Get the final expression (after all reductions).
    pub fn final_expr(&self) -> Option<&Expr> {
...
pub struct ReductionStats {
    ...
    /// Number of delta unfoldings performed.
    pub delta_steps: usize,
    /// Number of let reductions performed.
    pub let_steps: usize,
    /// Number of projection reductions performed.
    pub proj_steps: usize,
    /// Total number of steps.
    pub total_steps: usize,
    /// Whether the reduction was aborted.
    pub aborted: bool,
}
impl ReductionStats {
    ...
    /// Returns `true` if any reductions were performed.
    pub fn any_reductions(&self) -> bool {
        self.total_steps > 0
    }
...
impl ReductionBound {
    /// Returns `true` if the bound has been exceeded.
    pub fn exceeded(&self, steps: usize, size: usize) -> bool {
        match self {
            ReductionBound::Unlimited => false,
            ReductionBound::Steps(n) => steps >= *n,
            ReductionBound::Size(n) => size >= *n,
        }
    }
}
...
pub enum ReductionStrategy {
    ...
    ///
    /// Only reduces the outermost redex; sub-expressions may remain unreduced.
    /// Efficient for most type-checking purposes.
    WHNF,
...
impl ReductionStrategy {
    ...
    pub fn name(&self) -> &'static str {
        match self {
            ReductionStrategy::WHNF => "whnf",
            ReductionStrategy::NF => "nf",
            ReductionStrategy::OneStep => "one-step",
            ReductionStrategy::CBV => "cbv",
            ReductionStrategy::CBN => "cbn",
            ReductionStrategy::HeadOnly => "head-only",
        }
    }
    /// Returns `true` if this strategy reduces to a complete normal form.
    pub fn is_complete(&self) -> bool {
        matches!(self, ReductionStrategy::NF)
    }
    /// Returns `true` if this is a lazy strategy.
    pub fn is_lazy(&self) -> bool {
        matches!(
            self,
            ReductionStrategy::WHNF | ReductionStrategy::CBN | ReductionStrategy::HeadOnly
        )
    }
    /// Returns `true` if this is an eager/strict strategy.
    pub fn is_eager(&self) -> bool {
        matches!(self, ReductionStrategy::CBV)
    }
}
