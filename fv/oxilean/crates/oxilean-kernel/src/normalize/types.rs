...
pub enum NormStrategy {
    ...
    /// Only WHNF (outermost).
    Whnf,
    /// Full normalization (under binders).
    Full,
    /// Bounded normalization (up to a depth limit).
    Bounded(usize),
}
...
///
/// This is useful when normalizing large expressions with shared subterms.
#[allow(dead_code)]
pub struct MemoizedNormalizer {
    pub(super) cache: HashMap<String, Expr>,
}
impl MemoizedNormalizer {
    /// Create a new memoized normalizer with an empty cache.
    #[allow(dead_code)]
    pub fn new() -> Self {
        MemoizedNormalizer {
            cache: HashMap::new(),
    ...
    /// Normalize the given expression, caching the result.
    #[allow(dead_code)]
...
///
/// The expression is stored un-normalized and only computed on first access.
pub struct LazyNormal {
    pub(super) original: Expr,
    pub(super) normal: std::cell::OnceCell<Expr>,
}
impl LazyNormal {
    /// Wrap an expression without normalizing.
    pub fn new(e: Expr) -> Self {
        Self {
            original: e,
            normal: std::cell::OnceCell::new(),
        }
    }
    /// Get the original expression without normalizing.
    pub fn original(&self) -> &Expr {
        &self.original
    }
    /// Get the normalized expression (computed lazily).
    pub fn normalized(&self) -> &Expr {
        self.normal.get_or_init(|| normalize(&self.original))
    }
    /// Check if the expression has already been normalized.
    pub fn is_evaluated(&self) -> bool {
        self.normal.get().is_some()
    }
}
...
pub struct NormStats {
    /// Expressions visited.
    pub visited: usize,
    /// Cache hits (same expression normalized more than once).
    pub cache_hits: usize,
    /// Beta reductions performed.
    pub beta_steps: usize,
    /// Let unfoldings performed.
    pub let_steps: usize,
}
impl NormStats {
    ...
    pub fn total_steps(&self) -> usize {
        self.beta_steps + self.let_steps
    }
}
