...
//
// Radix-sort stable scatter (keys only). One workgroup per 256-element tile.
// Each lane computes its digit, its stable local rank within the tile (count of
// equal-digit predecessors), and writes the key to base[d*num_tiles+tile] +
// local_rank. Out-of-range lanes (gid >= n) do not write. Workgroup_size = 256.
...
@group(0) @binding(3) var<storage, read>       params: RadixParams;
