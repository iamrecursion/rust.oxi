//! Batched LLaMA prefill must be indistinguishable from the per-token loop.
//!
//! `LlamaModel::forward` now sends any multi-token call through the tiled path
//! in `llama::batch`, which processes `PREFILL_TILE` tokens per pass over the
//! weights instead of streaming the whole quantized weight set once per prompt
//! token.  That is only a safe default because the two paths agree **bit for
//! bit**: the CLI samples at temperature, so a one-ULP logit difference is a
//! different word on screen.
//!
//! This file pins the model half of that claim — RoPE positions, causal masking,
//! the KV staging/commit dance, the residual stream and the tiling seam — by
//! running a whole synthetic LLaMA checkpoint both ways.
//!
//! The projections are **Q4_K**, not F32, on purpose: the batched route is gated
//! on the weight kernel advertising a fused Q8_0 activation path, and only the
//! K-quants do.  An F32 fixture would make every assertion below a tautology,
//! which `q4_k_fixture_reaches_the_batched_path` guards against.
...
//   * ATTN_DIM != HIDDEN, so a buffer sized from the wrong one trips.
//   * HEADS != KV_HEADS, so grouped-query head mapping is exercised.
//   * FFN spans two Q4_K blocks, so `ffn_down`'s multi-block row loop runs.
...
fn build_fixture() -> Vec<u8> {
    let mut rng = Rng::new(0xBA7_C4ED);
    ...
    // Tied checkpoint: token_embd doubles as the LM head, which also keeps the
    // tied-fallback path inside the batched test's blast radius.
...
///
/// 40 tokens = 2 full tiles + a ragged 8, so the tiling seam is covered.
#[test]
...
/// path would have written — otherwise the *next* token decodes differently even
/// though the prefill logits matched.
#[test]
...
/// Prompts shorter than, equal to and longer than one tile.
#[test]
