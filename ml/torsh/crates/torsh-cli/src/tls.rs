...
/// A [`reqwest::ClientBuilder`] pre-configured with the pure-Rust TLS stack.
...
mod tests {
    ...
    fn rustls_config_builds_with_webpki_roots() {
        ...
        // The Mozilla root set is non-empty and ALPN advertises h2 + http/1.1.
    ...
    fn client_builder_builds_a_client() {
        // Proves the provider + roots are wired correctly and the client builds
        // without hitting the `rustls-no-provider` panic path.
        let client = client_builder()
            .expect("builder")
            .build()
            .expect("client must build with the pure-Rust TLS stack");
        drop(client);
    }
}
