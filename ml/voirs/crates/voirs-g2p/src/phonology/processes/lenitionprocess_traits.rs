//! # LenitionProcess - Trait Implementations
//!
//! This module contains trait implementations for `LenitionProcess`.
//!
//! ## Implemented Traits
//!
//! - `PhonologicalProcess`
...
impl PhonologicalProcess for LenitionProcess {
    fn apply(&self, phonemes: &[Phoneme], config: &ProcessConfig) -> Result<Vec<Phoneme>> {
        ...
        for i in 0..phonemes.len() {
            ...
            if prev_is_vowel && next_is_vowel && config.aggressiveness > 0.4 {
                if let Some(lenited) = self.lenite(phoneme) {
                    result.push(lenited);
                    continue;
                }
            }
