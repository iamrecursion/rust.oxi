...
//! OpenSSL, no `aws-lc-sys`, no vendored C signing library).
//!
//! Every function here is a small, independently testable piece of the
//! algorithm rather than one opaque "sign this request" black box, so a
//! reviewer can check each step (URI encoding, canonical headers, canonical
//! request, string-to-sign, signing-key derivation) against the AWS spec in
//! isolation.
...
fn hmac_sha256(key: &[u8], data: &[u8]) -> Result<Vec<u8>> {
    let mut mac = Hmac::<Sha256>::new_from_slice(key)
        .map_err(|e| DatasetError::CloudStorage(format!("HMAC initialization failed: {e}")))?;
    mac.update(data);
...
/// Build the SigV4 `CanonicalQueryString`: each parameter name and value is
/// URI-encoded independently (escaping `/`), then pairs are sorted by
/// encoded name with ties broken by encoded value, and joined as
/// `key=value` pairs separated by `&`. An empty slice yields an empty
/// string.
pub(super) fn canonical_query_string(params: &[(String, String)]) -> String {
...
mod tests {
    ...
    fn canonical_query_string_sorts_by_encoded_key_then_value() {
        ...
        assert_eq!(canonical_query_string(&params), "list-type=2&prefix=a%2Fb");
    ...
    fn canonical_request_matches_hand_verified_structure() {
        // A minimal, hand-constructed example (not copied from any external
        // source) that exercises every canonical-request component: a
        // `/`-preserving path, an empty query string, three signed headers
        // in sorted order, and the well-known SHA-256("") payload hash used
        // for bodiless requests. Every line below can be diffed by eye
        // against the SigV4 canonical-request definition.
        let mut headers = BTreeMap::new();
        headers.insert(
            "host".to_string(),
...
e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855";
    ...
    fn signature_is_deterministic_and_sensitive_to_every_input() {
        ...
        assert_ne!(
            base,
            signature("secret", "20130524", "us-east-1", "glacier", "sts").unwrap()
        );
        assert_ne!(
            base,
            signature("secret", "20130524", "us-east-1", "s3", "different-sts").unwrap()
        );
