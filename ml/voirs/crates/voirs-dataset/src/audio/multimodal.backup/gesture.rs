...
impl GestureAnalysisResult {
    ...
    pub fn calculate_quality_score(&mut self) {
        ...
        let correlation_strength: f32 = self.correlations.iter().map(|c| c.correlation_strength).sum();
...
impl DetectedGesture {
    ...
    pub fn new(id: String, start_time: f32, end_time: f32, category: GestureCategory, confidence: f32) -> Self {
        Self {
            id,
            start_time,
            end_time,
