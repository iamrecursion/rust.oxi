...
async fn test_complete_pipeline_integration() -> Result<()> {
    ...
    timeout(TEST_TIMEOUT, async {
        ...
        let config = SynthesisConfig {
            ...
            language: voirs_sdk::LanguageCode::EnUs,
            effects: Vec::new(),
    ...
    .await
    .map_err(|_| voirs_sdk::VoirsError::timeout("Test timed out after 2 minutes"))?
}
...
async fn test_vits_acoustic_model() -> Result<()> {
    use voirs_integration_tests::{Phoneme, SynthesisConfig};
...
async fn test_hifigan_vocoder() -> Result<()> {
    use voirs_integration_tests::MelSpectrogram;
...
async fn test_synthesis_config_effects() -> Result<()> {
    ...
    .await
    .map_err(|_| {
        voirs_sdk::VoirsError::timeout("Synthesis config effects test timed out after 2 minutes")
    })?
}
...
async fn test_integration_various_text_types() -> Result<()> {
    ...
    .await
    .map_err(|_| {
        voirs_sdk::VoirsError::timeout("Various text types test timed out after 5 minutes")
    })?
}
...
async fn test_integration_streaming_synthesis() -> Result<()> {
    ...
    .await
    .map_err(|_| voirs_sdk::VoirsError::timeout("Streaming synthesis test timed out after 3 minutes"))?
}
...
async fn test_integration_performance_benchmark() -> Result<()> {
    ...
    .await
    .map_err(|_| voirs_sdk::VoirsError::timeout("Performance benchmark test timed out after 5 minutes"))?
}
...
async fn test_integration_component_integration() -> Result<()> {
    ...
    .await
    .map_err(|_| {
        voirs_sdk::VoirsError::timeout("Component integration test timed out after 2 minutes")
    })?
}
...
async fn test_integration_reproducibility() -> Result<()> {
    ...
    .await
    .map_err(|_| voirs_sdk::VoirsError::timeout("Reproducibility test timed out after 2 minutes"))?
}
...
async fn test_integration_error_handling() -> Result<()> {
    ...
    .await
    .map_err(|_| voirs_sdk::VoirsError::timeout("Error handling test timed out after 2 minutes"))?
}
