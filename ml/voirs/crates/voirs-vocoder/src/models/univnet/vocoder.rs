//! `Vocoder` trait implementation for [`UnivNetInference`].
//!
//! Bridges the low-level UnivNet generator API (tensor in / tensor out) with
//! the high-level [`Vocoder`] trait used by the VoiRS pipeline.

#[cfg(feature = "candle")]
use super::inference::UnivNetInference;
...
#[cfg(feature = "candle")]
#[async_trait]
impl Vocoder for UnivNetInference {
    /// Convert a [`MelSpectrogram`] to an [`AudioBuffer`] using UnivNet.
    ///
    ...
    async fn vocode(
        ...
        let mel_tensor =
            candle_core::Tensor::from_vec(flat, vec![1, mel.n_mels, mel.n_frames], self.device())
                .map_err(|e| {
                VocoderError::ModelError(format!("UnivNet: failed to build mel tensor: {}", e))
            })?;

        // Ensure tensor dtype is F32
        let mel_tensor = mel_tensor
            .to_dtype(DType::F32)
            .map_err(|e| VocoderError::ModelError(format!("UnivNet: dtype cast error: {}", e)))?;
        ...
        let samples = output
            .squeeze(0)
            .map_err(|e| VocoderError::ModelError(format!("UnivNet: squeeze(0) failed: {}", e)))?
            .squeeze(0)
            .map_err(|e| {
                VocoderError::ModelError(format!("UnivNet: squeeze(0) second call failed: {}", e))
            })?
            .to_vec1::<f32>()
            .map_err(|e| VocoderError::ModelError(format!("UnivNet: to_vec1 failed: {}", e)))?;
    ...
    fn metadata(&self) -> VocoderMetadata {
        VocoderMetadata {
            name: "UnivNet".to_string(),
            version: "0.1.0".to_string(),
            architecture: "UnivNet+LVC".to_string(),
...
mod tests {
    use super::*;
    use crate::models::univnet::config::UnivNetConfig;
    ...
    fn test_load_weights_rejects_no_match() {
        ...
        let config = UnivNetConfig::fast_24khz();
        let device = Device::Cpu;
        let mut inference = UnivNetInference::new(config, device).unwrap();
        ...
        let tmp_path = tmp_dir.join("univnet_test_no_match.safetensors");
    ...
    async fn test_metadata_values() {
        let config = UnivNetConfig::fast_24khz();
        let device = Device::Cpu;
        let inference = UnivNetInference::new(config.clone(), device).unwrap();

        let meta = inference.metadata();
        assert_eq!(meta.name, "UnivNet");
