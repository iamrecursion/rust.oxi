//! Metal 2-D convolution benchmark — measures `ComputeBackend::conv2d_forward`
//! on a typical mid-size feature map: batch 1, 16 input channels, 64×64
//! spatial, 32 output channels, a 3×3 kernel, stride 1, padding 1 (so the
//! output stays 64×64, `SAME`-style).
//!
//! ## Platform behaviour
//!
//! * **macOS with a Metal-capable GPU** — the benchmark runs.
//! * **No Metal device (headless CI, non-macOS)** — prints
//!   `skip: no Metal device (conv2d)` to stderr and returns before
//!   registering any benchmark closure with criterion.
//!
//! Run with:
//! ```bash
//! cargo bench -p oxicuda-metal --bench conv2d_bench
//! ```
...
fn bench_conv2d(criterion: &mut Criterion) {
    let harness = match try_setup() {
        Some(h) => h,
        None => {
            eprintln!("skip: no Metal device (conv2d)");
            return;
        }
    };

    let mut group = criterion.benchmark_group("metal_conv2d_1x16x64x64_k32x3x3");
