    ...
    for i in 0..20 {
        let hyperparams = HashMap::from([
            (
                ...
                0.001 * (1.0 + scirs2_core::random::thread_rng().random::<f64>()),
            ),
            (
                "weight_decay".to_string(),
                0.01 * scirs2_core::random::thread_rng().random::<f64>(),
            ),
            (
                "batch_size".to_string(),
                32.0 + scirs2_core::random::thread_rng().random::<f64>() * 96.0,
            ),
            (
                "momentum".to_string(),
                0.9 + scirs2_core::random::thread_rng().random::<f64>() * 0.09,
            ),
        ]);
    ...
    for generation in 0..25 {
        ...
        for i in num_elite..population.len() {
            ...
            for (param, value) in new_hyperparams.iter_mut() {
                let perturbation = 1.0 + (scirs2_core::random::thread_rng().random::<f64>() - 0.5) * 0.2;
                *value *= perturbation;
...
fn neural_predictor_example() -> Result<()> {
    ...
    for test_case in 0..5 {
        ...
        let random_hyperparams = HashMap::from([
            (
                "learning_rate".to_string(),
                scirs2_core::random::thread_rng().random::<f64>() * 0.01,
            ),
            (
                "weight_decay".to_string(),
                scirs2_core::random::thread_rng().random::<f64>() * 0.01,
            ...
            (
                "batch_size".to_string(),
                16.0 + scirs2_core::random::thread_rng().random::<f64>() * 112.0,
            ),
            (
                "momentum".to_string(),
                0.9 + scirs2_core::random::thread_rng().random::<f64>() * 0.09,
