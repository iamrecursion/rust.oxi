...
//!
//! GoogleCloudFunctionsProvider has no real Google Cloud Functions client wired up, so every operation returns
...
//! targets Google Cloud Functions therefore fails loudly instead of believing it shipped a
...
use super::types::{
    DeploymentResult, DetailedMetrics, GoogleCloudFunctionsProvider, ScalingConfig,
    ServerlessConfig, ServerlessMetrics,
};

const PROVIDER: &str = "GoogleCloudFunctionsProvider";
