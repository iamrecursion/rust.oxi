...
impl Adam8bit {
    ...
    pub fn step(
        ...
        for (name, param) in parameters.iter_mut() {
            ...
            let momentum_state = self.momentum_states.get_mut(name).ok_or_else(|| {
                anyhow::anyhow!("momentum_state should exist after initialization")
            })?;
            let variance_state = self.variance_states.get_mut(name).ok_or_else(|| {
                anyhow::anyhow!("variance_state should exist after initialization")
            })?;
