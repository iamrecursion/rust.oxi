...
use crate::param_id::ParamRegistry;
...
pub struct Adam {
    ...
    ///
    /// See [`crate::param_id`] for the identity contract that makes checkpoint
    /// resume work; the state maps above are keyed by the canonical keys it hands out.
...
impl Adam {
    /// Updates a parameter identified by a stable, caller-supplied `name`.
    ///
    /// Prefer this over [`Optimizer::update`] whenever the caller has parameter
    /// names: names are embedded in the checkpoint keys, which makes
    /// `state_dict` -> `load_state_dict` round trips independent of the order in
    /// which parameters are visited. See [`crate::param_id`] for the full contract.
    ///
    /// # Errors
    ///
    /// Returns an error when the tensor dtype is unsupported or a restored state
    /// buffer does not match the gradient size.
    pub fn update_named(
        &mut self,
        name: &str,
        ...
        let param_id = self.params.key_for_named_tensor(name, parameter)?;
        self.update_with_key(param_id, parameter, grad)
    }

    /// Shared update body, given an already-resolved stable parameter key.
    fn update_with_key(
        &mut self,
        param_id: String,
        parameter: &mut Tensor,
...
impl Optimizer for Adam {
    fn update(&mut self, parameter: &mut Tensor, grad: &Tensor) -> Result<()> {
        let param_id = self.params.key_for_tensor(parameter)?;
        self.update_with_key(param_id, parameter, grad)
    }
...
impl StatefulOptimizer for Adam {
    ...
    fn load_state_dict(&mut self, state: HashMap<String, Tensor>) -> Result<()> {
        ...
        // Restore the moment buffers *and* the parameter identity registry. The
        // registry is rebuilt purely from the checkpointed keys (see
        // [`crate::param_id`]); its slots start out unbound so the first `update()`
        // calls of the resuming run claim them in registration order. Restoring the
        // registry is what makes resume actually restore anything: without it the
        // keys generated by the new process would never match the checkpointed ones.
        //
        // Keys are visited in sorted order so that `p:0`, `p:1`, ... rebuild the
        // registry in the same order they were assigned, regardless of `HashMap`
        // iteration order.
        let mut moment_keys: Vec<&String> = state.keys().collect();
        moment_keys.sort();
        for key in moment_keys {
            let Some(tensor) = state.get(key) else {
                continue;
            };
            let (param_id, numel) = if let Some(id) = key.strip_prefix("exp_avg_sq_") {
                let buffer = tensor.data()?;
                let numel = buffer.len();
                self.exp_avg_sq.insert(id.to_string(), buffer);
                (id, numel)
            } else if let Some(id) = key.strip_prefix("exp_avg_") {
                let buffer = tensor.data()?;
                let numel = buffer.len();
                self.exp_avg.insert(id.to_string(), buffer);
                (id, numel)
            } else {
                continue;
            };
            self.params.restore_key(param_id, numel)?;
        }
