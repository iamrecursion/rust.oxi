...
impl GradContext {
    ...
    pub fn enable() -> Self {
        ...
        let previous_state =
            engine.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_grad_enabled();
        engine.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).enable_grad();
    ...
    pub fn disable() -> Self {
        ...
        let previous_state =
            engine.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_grad_enabled();
        engine.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).disable_grad();
