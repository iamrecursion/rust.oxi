//! # AttentionOptimizationRule - Trait Implementations
//!
//! This module contains trait implementations for `AttentionOptimizationRule`.
//!
//! ## Implemented Traits
//!
//! - `OptimizationRule`
...
use crate::visualization::GraphAnalyzer;
use std::collections::HashMap;
use std::fmt;

use super::functions::OptimizationRule;
use super::types::{
    AnalysisContext, AttentionOptimizationRule, BatchingRule, CachingRule, CodeExample, Difficulty,
    FlashAttentionRule, GradientCheckpointingRule, HardwareInfo, ImpactLevel, KernelFusionRule,
    MemoryOptimizationRule, MixedPrecisionRule, OptimizationAdvisor, OptimizationCategory,
    OptimizationSuggestion, ParallelizationRule, PerformanceImprovement, QuantizationRule,
};
                            ...
                            return Ok(
                                Some(OptimizationSuggestion {
                                    ...
                                    implementation_steps: vec![
                                        "Identify attention layers that process long sequences"
                                        .to_string(),
                                        "Replace with sparse attention (e.g., local, strided, or random patterns)"
                                        .to_string(),
                                        "Consider using Linformer, Performer, or similar linear attention variants"
                                        .to_string(), "Benchmark to ensure quality is maintained"
                                        .to_string(),
                                    ],
                                    code_examples: Some(
                                        vec![
                                            CodeExample { language : "rust".to_string(), description :
                                            "Using sparse attention".to_string(), code :
                                            r#"// Replace standard attention
...
impl OptimizationRule for BatchingRule {
    fn analyze(&self, context: &AnalysisContext) -> Result<Option<OptimizationSuggestion>> {
        if let Some(_latency) = &context.latency_metrics {
                ...
                if report.total_parameters > 100_000_000 {
                    return Ok(
                        Some(OptimizationSuggestion {
                            ...
                            implementation_steps: vec![
                                "Choose quantization method (INT8, INT4, or dynamic)"
                                .to_string(),
                            ...
                            code_examples: Some(
                                vec![
                                    CodeExample { language : "rust".to_string(), description :
                                    "Enabling INT8 quantization".to_string(), code :
                                    r#"use trustformers_core::quantization::{QuantizationConfig, quantize_int8};
...
let quantized_model = quantize_int8(&model, &config)?;"#
                                    .to_string(), }
                                ],
                            ),
                            warnings: vec![
