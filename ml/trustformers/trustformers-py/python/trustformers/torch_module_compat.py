...
class TorchModuleWrapper(nn.Module if nn else object):
    ...
    def save_pretrained(self, save_directory: str):
        ...
        Delegates to ``self.trustformers_model.save_pretrained(save_directory)``
        when available.  If the inner model does not expose this method, raises
        :class:`AttributeError` with guidance on what to implement.

        Args:
            save_directory: Directory path where the model will be saved.

        Raises:
            AttributeError: If the wrapped model does not implement
                ``save_pretrained``.
        """
        if hasattr(self.trustformers_model, 'save_pretrained'):
            self.trustformers_model.save_pretrained(save_directory)
        else:
            inner_type = type(self.trustformers_model).__name__
            raise AttributeError(
                f"{inner_type!r} does not implement 'save_pretrained'. "
                "To persist this model implement a 'save_pretrained(save_directory)' "
                "method on the inner model class, or serialise it manually before "
                "wrapping with TorchModuleWrapper."
            )
