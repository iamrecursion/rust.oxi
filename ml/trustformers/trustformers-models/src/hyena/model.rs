...
impl Model for HyenaModel {
    ...
    /// Loading a Hyena checkpoint is not implemented.
    ///
    /// # Errors
    ///
    /// Always fails.
    ...
    /// later. Hyena's implicit filter is generated by `HyenaFilter::generate_filter` from an exponential decay rather than read from a learned parameter, so a checkpoint's filter weights have nowhere to go in this implementation.
    fn load_pretrained(&mut self, reader: &mut dyn Read) -> Result<()> {
        ...
        std::io::Read::read_to_end(reader, &mut buffer).map_err(|e| {
            TrustformersError::io_error(format!("failed to read Hyena weights: {e}"))
        })?;

        Err(TrustformersError::not_implemented(
            "Hyena checkpoint loading is not implemented: Hyena's implicit filter is generated by `HyenaFilter::generate_filter` from an exponential decay rather than read from a learned parameter, so a checkpoint's filter weights have nowhere to go in this implementation. Install weights \
