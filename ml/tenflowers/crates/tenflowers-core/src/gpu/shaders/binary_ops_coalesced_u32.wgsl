// Enhanced binary operation compute shaders with memory coalescing optimizations for u32
...
var<workgroup> tile_a: array<u32, 256>; // 16x16 tile
var<workgroup> tile_b: array<u32, 256>; // 16x16 tile

// Optimized workgroup size for memory coalescing
// Using 2D workgroups for better memory access patterns
@compute @workgroup_size(16, 16, 1)
fn add_op_coalesced_u32(@builtin(global_invocation_id) global_id: vec3<u32>,
...
fn add_op_vectorized_u32(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let base_index = global_id.x * 4u; // Process 4 u32 elements at once
    ...
    // Vectorized memory access - process 4 u32 elements at once
    let a_base = base_index % arrayLength(&input_a);
    let b_base = base_index % arrayLength(&input_b);
    
    // Load 4 u32 elements at once for better memory bandwidth
...
@compute @workgroup_size(16, 16, 1)
fn sub_op_coalesced_u32(@builtin(global_invocation_id) global_id: vec3<u32>,
                        @builtin(local_invocation_id) local_id: vec3<u32>) {
...
@compute @workgroup_size(64, 1, 1)
fn sub_op_vectorized_u32(@builtin(global_invocation_id) global_id: vec3<u32>) {
...
@compute @workgroup_size(16, 16, 1)
fn mul_op_coalesced_u32(@builtin(global_invocation_id) global_id: vec3<u32>,
                        @builtin(local_invocation_id) local_id: vec3<u32>) {
...
@compute @workgroup_size(64, 1, 1)
fn mul_op_vectorized_u32(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let base_index = global_id.x * 4u;
...
@compute @workgroup_size(16, 16, 1)
fn div_op_coalesced_u32(@builtin(global_invocation_id) global_id: vec3<u32>,
                        @builtin(local_invocation_id) local_id: vec3<u32>) {
...
@compute @workgroup_size(64, 1, 1)
fn div_op_vectorized_u32(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let base_index = global_id.x * 4u;
    ...
    if (input_b[b_base + 2u] == 0u) {
        output[base_index + 2u] = 0u;
    } else {
        output[base_index + 2u] = input_a[a_base + 2u] / input_b[b_base + 2u];
    }
    
    if (input_b[b_base + 3u] == 0u) {
        output[base_index + 3u] = 0u;
    } else {
        output[base_index + 3u] = input_a[a_base + 3u] / input_b[b_base + 3u];
    }
}
...
@compute @workgroup_size(16, 16, 1)
fn pow_op_coalesced_u32(@builtin(global_invocation_id) global_id: vec3<u32>,
                        @builtin(local_invocation_id) local_id: vec3<u32>) {
...
@compute @workgroup_size(16, 16, 1)
fn prelu_op_coalesced_u32(@builtin(global_invocation_id) global_id: vec3<u32>,
                          @builtin(local_invocation_id) local_id: vec3<u32>) {
