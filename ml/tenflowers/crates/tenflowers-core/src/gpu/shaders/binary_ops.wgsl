// Binary operation compute shaders
...
// Enhanced binary operations with proper broadcasting

// Addition operation - optimized workgroup size for better GPU utilization
...
fn pow_op(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    output[index] = pow(input_a[a_idx], input_b[b_idx]);
}

// PReLU operation: input_a is x, input_b is alpha
// PReLU(x, alpha) = x if x > 0, else alpha * x
@compute @workgroup_size(256)
fn prelu_op(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (x > 0.0) {
        ...
        output[index] = alpha * x;
    }
}

// Element-wise minimum operation
@compute @workgroup_size(256)
fn min_op(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let index = global_id.x;
    if (index >= arrayLength(&output)) {
        return;
    }
    ...
    output[index] = min(input_a[a_idx], input_b[b_idx]);
}

// Element-wise maximum operation
@compute @workgroup_size(256)
fn max_op(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let index = global_id.x;
    if (index >= arrayLength(&output)) {
        return;
    }
    ...
    output[index] = max(input_a[a_idx], input_b[b_idx]);
}
...
fn pow_broadcast(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Perform operation
    output[output_idx] = pow(input_a[a_flat_idx], input_b[b_flat_idx]);
}
...
fn prelu_broadcast(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (x > 0.0) {
        ...
        output[output_idx] = alpha * x;
    }
}

// Element-wise minimum operation with broadcasting
@compute @workgroup_size(256)
fn min_broadcast(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let output_idx = global_id.x;
    if (output_idx >= arrayLength(&output)) {
        return;
    }
    ...
    // Get flat indices for inputs
    let a_flat_idx = compute_flat_index_broadcast(&a_indices, a_shape_offset, a_rank);
    let b_flat_idx = compute_flat_index_broadcast(&b_indices, b_shape_offset, b_rank);
    
    // Perform min operation
    output[output_idx] = min(input_a[a_flat_idx], input_b[b_flat_idx]);
}
...
fn max_broadcast(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    output[output_idx] = max(input_a[a_flat_idx], input_b[b_flat_idx]);
}