// Binary operation compute shaders for u64 type
...
fn div_op(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (input_b[b_idx] == 0u) {
        output[index] = 0u; // Or could use max value u64::MAX
    } else {
        output[index] = input_a[a_idx] / input_b[b_idx];
    }
}

// Power operation (using repeated multiplication for u64)
@compute @workgroup_size(64)
...
// PReLU operation (adapted for u64 - note: this is unusual for unsigned types)
@compute @workgroup_size(64)
