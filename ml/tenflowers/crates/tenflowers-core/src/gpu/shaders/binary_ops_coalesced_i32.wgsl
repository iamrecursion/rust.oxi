// Enhanced binary operation compute shaders with memory coalescing optimizations for i32
...
var<workgroup> tile_a: array<i32, 256>; // 16x16 tile
var<workgroup> tile_b: array<i32, 256>; // 16x16 tile

// Optimized workgroup size for memory coalescing
// Using 2D workgroups for better memory access patterns
@compute @workgroup_size(16, 16, 1)
fn add_op_coalesced_i32(@builtin(global_invocation_id) global_id: vec3<u32>,
...
fn add_op_vectorized_i32(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let base_index = global_id.x * 4u; // Process 4 i32 elements at once
    ...
    // Vectorized memory access - process 4 i32 elements at once
    let a_base = base_index % arrayLength(&input_a);
    let b_base = base_index % arrayLength(&input_b);
    
    // Load 4 i32 elements at once for better memory bandwidth
...
@compute @workgroup_size(16, 16, 1)
fn sub_op_coalesced_i32(@builtin(global_invocation_id) global_id: vec3<u32>,
                        @builtin(local_invocation_id) local_id: vec3<u32>) {
...
@compute @workgroup_size(16, 16, 1)
fn mul_op_coalesced_i32(@builtin(global_invocation_id) global_id: vec3<u32>,
                        @builtin(local_invocation_id) local_id: vec3<u32>) {
...
@compute @workgroup_size(16, 16, 1)
fn div_op_coalesced_i32(@builtin(global_invocation_id) global_id: vec3<u32>,
                        @builtin(local_invocation_id) local_id: vec3<u32>) {
...
@compute @workgroup_size(16, 16, 1)
fn rem_op_coalesced_i32(@builtin(global_invocation_id) global_id: vec3<u32>,
                        @builtin(local_invocation_id) local_id: vec3<u32>) {
...
@compute @workgroup_size(16, 16, 1)
fn and_op_coalesced_i32(@builtin(global_invocation_id) global_id: vec3<u32>,
                        @builtin(local_invocation_id) local_id: vec3<u32>) {
...
@compute @workgroup_size(16, 16, 1)
fn or_op_coalesced_i32(@builtin(global_invocation_id) global_id: vec3<u32>,
                       @builtin(local_invocation_id) local_id: vec3<u32>) {
...
@compute @workgroup_size(16, 16, 1)
fn xor_op_coalesced_i32(@builtin(global_invocation_id) global_id: vec3<u32>,
                        @builtin(local_invocation_id) local_id: vec3<u32>) {
