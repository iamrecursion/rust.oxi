//! Separable convolution layers implementation
//!
//! This module contains the SeparableConv2D layer implementation, which performs
//! Xception-style separable convolutions using depthwise convolution followed by
...
/// Separable 2D Convolutional layer implementing Xception-style separable convolutions
/// This performs depthwise convolution followed by pointwise convolution with flexible output channels
pub struct SeparableConv2D<T> {
...
impl<T> SeparableConv2D<T>
where
    T: Clone + Default + Zero,
{
    /// Create a new SeparableConv2D layer
    ///
    /// # Arguments
    /// * `in_channels` - Number of input channels
    /// * `out_channels` - Number of output channels (can be different from in_channels * depth_multiplier)
    ...
    pub fn new(
        ...
        let intermediate_channels = in_channels * depth_multiplier;

        // Depthwise weight shape: [in_channels * depth_multiplier, 1, kernel_height, kernel_width]
        let depthwise_weight =
            Tensor::zeros(&[intermediate_channels, 1, kernel_size.0, kernel_size.1]);

        // Pointwise weight shape: [out_channels, intermediate_channels, 1, 1]
        let pointwise_weight = Tensor::zeros(&[out_channels, intermediate_channels, 1, 1]);
    ...
    /// Xception-style constructor with depth multiplier of 1
    pub fn xception(in_channels: usize, out_channels: usize, kernel_size: (usize, usize)) -> Self {
        Self::new(
            in_channels,
            out_channels,
            kernel_size,
    ...
    /// Create a deep copy of this SeparableConv2D layer
...
impl<T> Clone for SeparableConv2D<T>
where
    T: Clone
        + Default
        + Zero
...
impl<T> Layer<T> for SeparableConv2D<T>
where
    T: Clone
        + Default
        + Zero
