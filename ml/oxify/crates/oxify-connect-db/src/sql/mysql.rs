...
fn build_mysql_args(params: &[Value]) -> Result<MySqlArguments> {
...
fn extract_mysql_value(row: &MySqlRow, idx: usize, type_name: &str) -> Value {
    match type_name {
        "BOOLEAN" => row
...
fn row_to_json(row: &MySqlRow) -> Value {
    ...
    for col in row.columns() {
        ...
        let val = extract_mysql_value(row, col.ordinal(), &type_name);
...
#[async_trait]
impl SqlExecutor for MySqlProvider {
...
mod tests {
    ...
    #[test]
    fn test_build_mysql_args_integer() {
        let params = vec![Value::Number(99.into())];
        let result = build_mysql_args(&params);
        assert!(result.is_ok());
    }
    ...
    #[test]
    fn test_build_mysql_args_string() {
        let params = vec![Value::String("world".to_string())];
        let result = build_mysql_args(&params);
        assert!(result.is_ok());
    }

    #[test]
    fn test_build_mysql_args_empty() {
        let result = build_mysql_args(&[]);
        assert!(result.is_ok());
    ...
    async fn test_mysql_health_check() {
        ...
        let provider = MySqlProvider::new(cfg).await.expect("connection failed");
        provider.health_check().await.expect("health check failed");
    }

    #[tokio::test]
    #[ignore = "Requires database"]
    async fn test_mysql_fetch_all() {
        let cfg = DbConfig::from_env().expect("DATABASE_URL must be set");
        let provider = MySqlProvider::new(cfg).await.expect("connection failed");
        let rows = provider
