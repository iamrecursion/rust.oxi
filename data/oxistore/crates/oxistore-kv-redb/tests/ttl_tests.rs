...
fn open_mem() -> RedbStore {
    RedbStore::open_in_memory().expect("open in-memory failed")
}
...
fn ttl_purge_expired_count() {
    ...
    // Non-TTL keys must still be present.
...
/// Default TTL methods on a struct implementing only the required KvStore methods
/// return Unsupported. We verify this via the trait default implementations.
#[test]
fn ttl_default_returns_unsupported() {
    use oxistore_core::StoreError;

    // The default KvStore::put_with_ttl returns Unsupported.
    // We can't easily instantiate a bare KvStore impl in this crate, so we
    // verify the error type by checking that the error message contains "unsupported"
    // or matches the variant.
