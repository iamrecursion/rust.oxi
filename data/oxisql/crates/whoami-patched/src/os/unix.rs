...
impl Target for Os {
    ...
    fn devicename(self) -> Result<OsString> {
        ...
        {
            // Pure-Rust device name on Apple targets: fall back to the POSIX
            // hostname (libc::gethostname) instead of pulling the macOS
            // SystemConfiguration objc binding. The COOLJAPAN Pure Rust Policy
            // v2 forbids that binding (it matches the `-sys` gate) in Role-A
            // crates; `libc` is an allowed system binding.
