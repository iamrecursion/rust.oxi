...
/// Resolves the self-referential size of a "big" (> 126-byte) SQLite
/// record header: the header starts with a varint holding its own total
/// length, so growing that varint (e.g. from 1 to 2 bytes) can itself push
/// the header past the next varint-length boundary, needing yet another
/// byte. Mirrors SQLite's `sqlite3VdbeMakeRecord()` fixup (`nHdr +=
/// nVarint; if (nVarint < varintLen(nHdr)) nHdr++;`), but as an explicit
/// fixed-point loop: keep growing the assumed varint length until it's
/// consistent with the size it implies. Each non-converging iteration
/// strictly grows `varint_len` (capped at 9), so this always terminates
/// quickly, even in pathological cases.
///
/// `tentative_header_size` is the sum of the serial-type varints for every
/// column, not including the length-prefix varint itself.
fn resolve_big_header_size(tentative_header_size: usize) -> usize {
    let mut varint_scratch = [0u8; 9];
    let mut varint_len = write_varint(&mut varint_scratch, tentative_header_size as u64);
    loop {
        let candidate_header_size = tentative_header_size + varint_len;
        let candidate_varint_len = write_varint(&mut varint_scratch, candidate_header_size as u64);
        if candidate_varint_len == varint_len {
            break candidate_header_size;
        }
        varint_len = candidate_varint_len;
    }
}
...
impl ImmutableRecord {
    ...
    pub fn from_registers(registers: &[Register]) -> Self {
        ...
        if header_size <= MIN_HEADER_SIZE {
            ...
            header_size = resolve_big_header_size(header_size);
        }
        // 1. write header size
        let mut buf = Vec::new();
...
impl IndexKeySortOrder {
    pub fn get_sort_order_for_col(&self, column_idx: usize) -> SortOrder {
        // Only the first 64 key columns carry an explicit ASC/DESC bit (this is a
        // `u64` bitfield). Columns beyond that fall back to ASC -- the same value
        // the zero-initialized bitfield yields for any unset bit. This affects
        // only indexes/covering-index key specs wider than 64 columns (e.g. a
        // covering scan over a 74-column table), where the extra columns are
        // payload, not ordering keys.
        if column_idx >= 64 {
    ...
    pub fn from_index(index: &Index) -> Self {
        let mut spec = 0;
        for (i, column) in index.columns.iter().enumerate().take(64) {
    ...
    pub fn from_list(order: &[SortOrder]) -> Self {
        let mut spec = 0;
        for (i, order) in order.iter().enumerate().take(64) {
