# Contributing to OxiSQL

Thank you for your interest in contributing. OxiSQL is part of the
**COOLJAPAN ecosystem** (https://github.com/cool-japan/oxisql), a
...
- **Pure Rust by default.** No new C/C++/Fortran dependency, and no
  ...
  — notably `rusqlite` is never used; use `oxisql-sqlite-compat`
  instead.
- **No panics on untrusted input.** Do not add `.unwrap()`, `.expect()`,
  `panic!()`, `unreachable!()`, or `assert!()` on data derived from
  untrusted input (including query text and stored row data) outside
  of test code; return the crate's existing typed error instead.
