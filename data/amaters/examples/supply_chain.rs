...
async fn main() -> anyhow::Result<()> {
    ...
    for (id, co2, reliability, cost, compliance) in suppliers_a {
        ...
        company_a.put(&Key::from_str(&format!("{prefix}:co2")),        &encrypt_u8(*co2)).await?;
        company_a.put(&Key::from_str(&format!("{prefix}:reliability")), &encrypt_u8(*reliability)).await?;
        company_a.put(&Key::from_str(&format!("{prefix}:cost")),       &encrypt_u8(*cost)).await?;
        company_a.put(&Key::from_str(&format!("{prefix}:compliance")), &encrypt_u8(*compliance)).await?;
    ...
    for (id, co2, reliability, cost, compliance) in suppliers_b {
        ...
        company_b.put(&Key::from_str(&format!("{prefix}:co2")),        &encrypt_u8(*co2)).await?;
        company_b.put(&Key::from_str(&format!("{prefix}:reliability")), &encrypt_u8(*reliability)).await?;
        company_b.put(&Key::from_str(&format!("{prefix}:cost")),       &encrypt_u8(*cost)).await?;
        company_b.put(&Key::from_str(&format!("{prefix}:compliance")), &encrypt_u8(*compliance)).await?;
    ...
    println!("  Result type: {} (remains encrypted)", co2_circuit.result_type);
    ...
    println!("  Company A: {} encrypted fields across {} suppliers",
        company_a.len(), suppliers_a.len());
    println!("  Company B: {} encrypted fields across {} suppliers",
        company_b.len(), suppliers_b.len());
