...
#[test]
fn test_char_unicode_roundtrip() {
    let chars = ['α', 'β', 'γ', '日', '本', '語', '🦀', '∞', '€'];
    for &c in &chars {
        let enc = encode_to_vec(&c).expect("encode");
        let (dec, _): (char, _) = decode_from_slice(&enc).expect("decode");
        assert_eq!(c, dec);
...
#[test]
fn test_char_encodes_as_u32_codepoint() {
    // 'A' = U+0041 = codepoint 65
    let enc = encode_to_vec(&'A').expect("encode");
    // Decode as u32 - should be 65
    let (code, _): (u32, _) = decode_from_slice(&enc).expect("decode as u32");
