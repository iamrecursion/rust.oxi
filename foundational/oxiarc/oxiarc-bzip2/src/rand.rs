//! De-randomisation support for legacy randomised BZip2 blocks.
//!
//! bzip2 versions up to and including 0.9.0 could "randomise" highly
//! repetitive blocks to work around bad sorting performance: the RLE1 data
//! was XOR-ed with 1 at pseudo-random positions before the BWT, and the
//! block header's randomised bit was set. The feature was deprecated in
//! bzip2 0.9.5, but libbz2 still *decodes* such blocks, so a conformant
//! decoder must too.
//!
//! The pseudo-random schedule is driven by libbz2's fixed 512-entry
//! `BZ2_rNums` table (randtable.c, bzip2 1.0.8): a countdown is loaded from
//! the table (cycling through it) and the byte at which the countdown
//! reaches exactly 1 is XOR-ed with 1. Applying the same pass again undoes
//! it, so [`derandomise`] serves both directions.

/// libbz2's `BZ2_rNums` randomisation schedule (randtable.c, bzip2 1.0.8).
const RNUMS: [u16; 512] = [
...
/// XOR-derandomise `data` in place using libbz2's schedule.
///
/// Mirrors libbz2's `BZ_RAND_INIT_MASK` / `BZ_RAND_UPD_MASK` /
/// `BZ_RAND_MASK` macros: for every byte the countdown is reloaded from
/// [`RNUMS`] when it hits zero and decremented; the byte is XOR-ed with 1
/// exactly when the countdown lands on 1. The transform is an involution,
/// so the encoder-side randomisation pass is this same function.
pub(crate) fn derandomise(data: &mut [u8]) {
    let mut n_to_go: i32 = 0;
    let mut t_pos: usize = 0;
    for byte in data.iter_mut() {
        if n_to_go == 0 {
            n_to_go = i32::from(RNUMS[t_pos]);
            t_pos = (t_pos + 1) % RNUMS.len();
        }
        n_to_go -= 1;
        *byte ^= u8::from(n_to_go == 1);
    }
}
...
mod tests {
    ...
    #[test]
    fn table_matches_libbz2_spot_checks() {
        // First row, last entries and length pin the exact libbz2 table.
        assert_eq!(RNUMS.len(), 512);
        assert_eq!(
            &RNUMS[0..10],
            &[619, 720, 127, 481, 931, 816, 813, 233, 566, 247]
        );
        assert_eq!(&RNUMS[510..512], &[936, 638]);
        assert!(RNUMS.iter().all(|&n| (50..=999).contains(&n)));
    }

    #[test]
    fn derandomise_is_an_involution() {
        let original: Vec<u8> = (0..4096u32).map(|i| (i % 251) as u8).collect();
        let mut data = original.clone();
        derandomise(&mut data);
        assert_ne!(data, original, "the mask must flip at least one bit");
        derandomise(&mut data);
        assert_eq!(data, original);
    }

    #[test]
    fn first_flip_position_matches_schedule() {
        // rNums[0] == 619: countdown 619 -> the byte processed when the
        // counter reaches 1 is index 617 (0-based), nothing before it.
        let mut data = vec![0u8; 1000];
        derandomise(&mut data);
        let first_flip = data.iter().position(|&b| b == 1);
        assert_eq!(first_flip, Some(617));
    }
}
