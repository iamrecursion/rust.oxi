...
impl<'a> Compressor<'a> {
    ...
    pub fn compress_to_buffer<C: WriteBuf + ?Sized>(
        ...
        let out = oxiarc_zstd::compress_with_level(source, self.effective_level())
    ...
    pub fn compress(&mut self, data: &[u8]) -> std::io::Result<Vec<u8>> {
        oxiarc_zstd::compress_with_level(data, self.effective_level())
            .map_err(std::io::Error::other)
    }
}
...
impl<'a> Decompressor<'a> {
    ...
    pub fn upper_bound(data: &[u8]) -> Option<usize> {
        crate::zstd_safe::get_frame_content_size(data)
            .ok()
            .flatten()
...
mod tests {
    ...
    fn regression_decompress_to_buffer_slice_rejects_before_decoding() {
        ...
        let err = decompress_to_buffer(&frame, &mut dst)
            .expect_err("must reject oversized declared size");
        assert_eq!(err.kind(), std::io::ErrorKind::InvalidData);
    }
    ...
    fn regression_decompressor_decompress_to_buffer_succeeds_when_reserved() {
        ...
        let n = dec
            .decompress_to_buffer(&frame, &mut dst)
            .expect("must succeed with sufficient reservation");
    ...
    fn regression_compress_decompress_to_buffer_accept_positioned_cursor() {
        ...
        let compressed_len = {
            ...
            comp.compress_to_buffer(&data, &mut cursor)
                .expect("compress via cursor")
        };
        ...
        let decompressed_len = {
            ...
            dec.decompress_to_buffer(&cbuf[coffset..], &mut cursor)
                .expect("decompress via cursor")
        };
