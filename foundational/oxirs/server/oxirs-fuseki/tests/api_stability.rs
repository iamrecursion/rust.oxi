//! API stability regression tests for oxirs-fuseki.
//!
//! These tests guard against unintentional breaking changes in the public API
//! surface of `oxirs-fuseki`.  A committed `api_baseline.json` snapshot is
//! compared against the current `src/lib.rs` on every test run.
//!
//! To update the baseline after an *intentional* API change, run:
//! ```text
//! cargo run -p oxirs-fuseki --bin fuseki_api_snapshot --quiet \
//!     > server/oxirs-fuseki/api_baseline.json
//! ```

use oxirs_fuseki::api_surface::{diff_surfaces, parse_lib, ApiSurface, TypeSig};
use std::path::Path;
    ...
    let baseline_json = std::fs::read_to_string(&baseline_path).unwrap_or_else(|_| {
        panic!(
            "api_baseline.json not found at {} — \
             run `cargo run -p oxirs-fuseki --bin fuseki_api_snapshot --quiet \
             > server/oxirs-fuseki/api_baseline.json`",
...
fn api_surface_parse_fixture() {
    let dir = std::env::temp_dir().join("oxirs_fuseki_api_surface_test");
...
fn api_surface_changed_fn_is_breaking() {
    use oxirs_fuseki::api_surface::FnSig;
