// OXITEXT MODIFICATION (oxitext 0.2.2): 1 `.unwrap()` site converted to `?`
// (D13 -- a font parser reads untrusted bytes and must not panic on them).
// See ../../PROVENANCE.md.
use super::super::Tag;
...
impl<'a> Iterator for WritingSystems<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            // `self.langs` was just assigned above when it was `None`, so the
            // `else` arm is unreachable; it ends iteration rather than panics.
            let langs = self.langs.as_mut()?;
            if let Some(lang) = langs.next() {
