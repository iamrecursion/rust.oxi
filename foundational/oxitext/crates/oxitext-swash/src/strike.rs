// OXITEXT MODIFICATION (oxitext 0.2.2): 7 `manual_div_ceil` sites,
// `(x + 7) / 8` -> `x.div_ceil(8)`, machine-applied by `cargo clippy --fix`
// (stable since 1.73, under the 1.89 MSRV); plus the `cfg` correction marked
// inline below. See ../PROVENANCE.md.
...
#[cfg(feature = "scale")]
use alloc::vec::Vec;
// OXITEXT MODIFICATION (oxitext 0.2.2): added `not(feature = "std")` -- with
// both `std` and `libm` on, std's inherent float methods shadow `CoreFloat`'s
// and this import is dead, so `--all-features` (which the OxiText gate runs
// under `-D warnings`) went red on an unused import. See ../PROVENANCE.md.
#[cfg(all(feature = "libm", not(feature = "std"), feature = "scale"))]
