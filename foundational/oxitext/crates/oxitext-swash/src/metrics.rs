...
// `map_or` (D13 -- a font parser reads untrusted bytes and must not panic on
...
impl MetricsProxy {
    ...
    fn fill(&mut self, font: &FontRef) -> Option<()> {
        ...
        if vmtx != 0 {
            // `vmtx != 0` implies `vhea.is_some()` (see above); the fallback
            // matches the `hmtx_count` default a few lines up.
            let long_count = vhea.map_or(1, |t| t.num_long_metrics());
