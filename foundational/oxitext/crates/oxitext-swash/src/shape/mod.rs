// OXITEXT MODIFICATION (oxitext 0.2.2): doctest imports rewritten from `use
// swash::` to `use oxitext_swash::` (10 sites) -- the vendored crate keeps the
// default lib name `oxitext_swash`, so upstream's doctests would not resolve.
// Also modified here (oxitext 0.2.2): the 6 `self.store.unwrap()` sites were
// converted to `let ... else` / `if let` (D13 -- a font parser reads
// untrusted bytes and must not panic on them).
// See ../../PROVENANCE.md.
/*!
...
```
# use oxitext_swash::{FontRef, CacheKey, shape::*, text::Script};
...
```
# use oxitext_swash::{FontRef, CacheKey, shape::*, text::Script, tag_from_bytes};
# let font: FontRef = FontRef { data: &[], offset: 0, key: CacheKey::new() };
// let font = ...;
let mut context = ShapeContext::new();
let mut shaper = context.builder(font)
...
```
use oxitext_swash::text::cluster::{CharCluster, CharInfo, Parser, Token};
# use oxitext_swash::{FontRef, CacheKey, shape::*, text::Script, tag_from_bytes};
# let font: FontRef = FontRef { data: &[], offset: 0, key: CacheKey::new() };
# let mut context = ShapeContext::new();
let mut shaper = context.builder(font)
...
```
use oxitext_swash::text::{analyze, Script};
use oxitext_swash::text::cluster::{CharInfo, Parser, Token};
...
use oxitext_swash::text::cluster::{CharCluster, Status};
    ...
    pub fn add_cluster(&mut self, cluster: &CharCluster) {
        ...
        match self.engine.mode {
            ...
            EngineMode::Myanmar => {
                ...
                // Complex/Myanmar mode is only selected when a feature store
                // exists (`engine.rs`: `gsub.lang != 0 && script.is_complex()`).
                // If it somehow does not, degrade to the Simple-mode push rather
                // than panic on untrusted font data -- the cluster still ships.
    ...
    fn finish(&mut self) {
        ...
        match e.mode {
            EngineMode::Simple => match e.sub_mode {
                SubMode::Gsub => {
                    // No feature store means no GSUB to apply; the buffer is
                    // still positioned below.
