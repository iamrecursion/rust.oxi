// OXITEXT MODIFICATION (oxitext 0.2.2): 1 `.unwrap()` site converted (D13 -- a
// font parser reads untrusted bytes and must not panic on them); 3
// `sort_unstable_by(|a, b| a.0.cmp(&b.0))` -> `sort_unstable_by_key(|a| a.0)`
// and 1 `explicit_counter_loop` from the clippy conformance pass.
// See ../../PROVENANCE.md.
    ...
    fn build_stage(
        ...
        for (index, feature, mask) in &self.indices {
            if last_index == Some(*index) {
                // `last_index == Some(*index)` means the previous iteration
                // pushed a lookup; an empty cache simply skips the alias.
                let Some(mut lookup) = cache.lookups.last().copied() else {
                    continue;
                };
...
impl<'a, 'b, 'c> ApplyContext<'a, 'b, 'c> {
    ...
    fn collect_sequence(&mut self, len: usize) -> bool {
        ...
        for (i, g) in (self.s.cur + 1..).zip(&self.buf.glyphs[self.s.cur + 1..self.s.end]) {
            if !g.skip {
