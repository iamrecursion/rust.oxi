...
// swash::` to `use oxitext_swash::` (8 sites) -- the vendored crate keeps the
// default lib name `oxitext_swash`, so upstream's doctests would not resolve.
// Also here: `derivable_impls` on `Default for Source`, one `unused_mut`, and
// the `cfg` correction marked inline below.
// See ../../PROVENANCE.md.
/*!
...
hinting enabled:
```
# use oxitext_swash::{FontRef, CacheKey, scale::*};
...
```
# use oxitext_swash::{scale::{*, image::Image}, FontRef, GlyphId};
fn render_glyph(
...
// OXITEXT MODIFICATION (oxitext 0.2.2): added `not(feature = "std")` to this
// `cfg` -- see ../../PROVENANCE.md (same dead-import fix as `strike.rs`).
#[cfg(all(feature = "libm", not(feature = "std"), feature = "render"))]
