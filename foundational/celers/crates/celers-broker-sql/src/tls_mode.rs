...
///
/// This crate (`celers-broker-sql`) is MySQL-only — the Postgres half of
/// this module (copied verbatim from `celers-cli`'s dual-backend
/// `tls_mode.rs` per the established migration pattern) is unused here.
#[allow(dead_code)]
...
///
/// Unused in this MySQL-only crate — see [`PgTlsWanted`]'s doc comment.
#[allow(dead_code)]
#[must_use]
