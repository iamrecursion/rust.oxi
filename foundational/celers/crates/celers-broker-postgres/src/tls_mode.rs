...
//! `sslmode=` query parameter embedded in a database connection URL: a
//! caller who wrote `sslmode=require` in their connection string got a
//! TLS-encrypted connection, and a caller who wrote nothing (or
//! `sslmode=disable`) got plain-text.
//!
//! The `sqlx` → `oxisql` migration replaced every `sqlx::PgPoolOptions`
//! call in this crate's connection paths with
//! `oxisql_postgres::PgConnection::connect` /
//! `PgConnection::connect_with_timeout`, both of which take an explicit
//! [`oxisql_postgres::TlsMode`] value as an argument rather than inferring
//! one from the URL. All three call sites in this crate
//! (`broker_core.rs`'s `with_queue` and `with_pool_config` constructors,
//! and `notifications.rs`'s dedicated LISTEN connection in
//! `create_notification_listener`) were migrated by hardcoding
//! `TlsMode::Disabled` — silently downgrading any connection to plain-text
//! even when the URL explicitly asked for TLS via `sslmode=require`. That
//! is a MEDIUM-severity regression: a caller who believes they are
//! encrypting traffic (because their connection string says so) is
//! unknowingly sending credentials and query data in the clear.
//!
//! This module restores the pre-migration behavior by parsing the caller's
//! connection URL for the `sslmode` query parameter and building the
//! matching [`oxisql_postgres::TlsMode`] value. `TlsMode::Disabled` remains
//! a legitimate outcome — it is simply now an explicit, URL-driven choice
//! rather than a blind default hardcoded at every `connect()` call site.
//!
//! This mirrors `celers-cli`'s `src/tls_mode.rs`, which already fixed the
//! identical regression pattern for that crate's own `oxisql-postgres` /
//! `oxisql-mysql` call sites. Only the PostgreSQL half is needed here —
//! this crate has no MySQL connections.
//!
//! # Rules
//!
...
//! semantics: this crate's connection helpers have exactly one shot at
