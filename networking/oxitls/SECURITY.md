...
OxiTLS follows the COOLJAPAN ecosystem's rolling-release model. Only
...
to the latest release before reporting an issue to confirm it is still
reproducible.

## Reporting a vulnerability

Please **do not** file a public GitHub issue for a suspected security
vulnerability. As a TLS implementation, undisclosed vulnerabilities
...
This policy covers the OxiTLS crates published from this repository
(https://github.com/cool-japan/oxitls), including the `oxitls` facade,
`oxitls-core`, `oxitls-adapter-*` providers, `oxitls-webpki-roots`,
`oxitls-native-certs`, `oxitls-rcgen`, and `oxitls-h2`. Vulnerabilities
in upstream dependencies (`rustls`, RustCrypto crates, etc.) should be
