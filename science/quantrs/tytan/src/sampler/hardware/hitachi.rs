...
impl Sampler for HitachiCMOSSampler {
    ...
    fn run_hobo(
        ...
        // Enforce Hitachi CMOS hardware capacity (king-graph physical limit).
        let king_size = match &self.config.hardware_version {
            HardwareVersion::Gen4 { king_graph_size } => *king_graph_size,
            HardwareVersion::Gen5 {
                king_graph_size, ..
        ...
        if qubo.shape()[0] > king_size {
            return Err(SamplerError::InvalidModel(format!(
                "HOBO quadratization produced {} variables, which exceeds Hitachi CMOS capacity {}",
                qubo.shape()[0],
                king_size
            )));
        }
