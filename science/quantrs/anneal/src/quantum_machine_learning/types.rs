...
impl QuantumNeuralLayer {
    ...
    fn apply_activation(&self, x: f64) -> QmlResult<f64> {
        match self.activation {
            ...
            ActivationType::QuantumSigmoid => Ok(0.5 * (1.0 + (x * PI / 2.0).sin())),
            ActivationType::QuantumReLU => Ok(if x > 0.0 { x } else { 0.0 }),
            ActivationType::QuantumTanh => Ok((x * PI / 4.0).sin()),
        }
    }
}
