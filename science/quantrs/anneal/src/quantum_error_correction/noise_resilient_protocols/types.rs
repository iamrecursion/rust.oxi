...
use crate::quantum_error_correction::config::{QECResult, QuantumErrorCorrectionError};
use crate::quantum_error_correction::error_mitigation::AnnealingResult;
use crate::quantum_error_correction::logical_encoding::{
    LogicalAnnealingEncoder, LogicalEncodingResult,
};
...
impl NoiseResilientAnnealingProtocol {
    ...
    pub fn encode_problem(
        ...
        use crate::quantum_error_correction::codes::{CodeParameters, ErrorCorrectionCode};
        use crate::quantum_error_correction::logical_encoding::{
            EncodingOptimizationStrategy, HardwareIntegrationMode, LogicalEncoderConfig,
        };
        let code = ErrorCorrectionCode::SurfaceCode;
        let num_logical = problem.num_variables;
        let parameters = CodeParameters {
            distance: 3,
            num_logical_qubits: num_logical,
            num_physical_qubits: num_logical * 9,
            num_ancilla_qubits: num_logical * 8,
            code_rate: 1.0 / 9.0,
    ...
    fn select_history_based_protocol(&self) -> QECResult<AnnealingProtocol> {
        ...
        best_protocol
            .or_else(|| self.protocol_selector.available_protocols.first().cloned())
            .ok_or_else(|| {
                QuantumErrorCorrectionError::CodeError("No protocols available".to_string())
            })
    }
    ...
    fn apply_protocol_modifications(
        ...
        match protocol.protocol_type {
            ProtocolType::AdaptivePauseQuench => {
                modified_params.initial_temperature = params.initial_temperature * 0.5;
    ...
    fn estimate_thermal_noise_probability(&self) -> f64 {
        let thermal_energy = 8.617e-5 * self.noise_model.thermal_temperature;
        let interaction_energy = 1e-6;
        if thermal_energy > 0.0 {
            (thermal_energy / interaction_energy).min(0.1)
        } else {
            0.0
        }
    }
    ...
    fn adapt_parameters(
        ...
        let success = match self.adaptation_strategy.adaptation_algorithm {
            ...
            _ => self.apply_simple_adaptation(params, error_rate),
        };
        Ok(AdaptationEvent {
            timestamp,
            trigger,
    ...
    fn apply_threshold_based_adaptation(
        ...
        if error_rate > self.config.error_threshold {
            ...
            params.initial_temperature *= 0.9;
    ...
    fn apply_simple_adaptation(&self, params: &mut AnnealingParams, error_rate: f64) -> bool {
        if error_rate > self.config.error_threshold {
            params.initial_temperature *= 0.95;
