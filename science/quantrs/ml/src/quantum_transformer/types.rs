...
impl QuantumFeedForward {
    ...
    fn apply_quantum_activation(&self, input: &Array1<f64>) -> Result<Array1<f64>> {
        match self.activation {
            ActivationType::QuantumReLU => Ok(input.mapv(|x| if x > 0.0 { x } else { 0.0 })),
            ActivationType::QuantumGELU => Ok(input.mapv(|x| {
                let gelu = 0.5 * x * (1.0 + (x * 0.7978845608 * (1.0 + 0.044715 * x * x)).tanh());
                gelu
            })),
            ActivationType::QuantumSwish => Ok(input.mapv(|x| x / (1.0 + (-x).exp()))),
            ActivationType::ParameterizedQuantum => Ok(input.mapv(|x| (x * PI / 2.0).sin())),
            ActivationType::ClassicalHybrid => Ok(input.mapv(|x| x.tanh())),
        }
    }
    /// Apply dropout
