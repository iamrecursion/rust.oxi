...
impl DeviceTranspiler {
    ...
    /// Decompose a gate into a native gate set.
    ///
    /// Supported decompositions (targeting {CX/CNOT, RZ, RY, H}):
    /// - T      → RZ(π/4)
    /// - T†     → RZ(-π/4)
    /// - S      → RZ(π/2)
    /// - SWAP   → 3 × CNOT
    /// - CZ     → H – CNOT – H  (on the target)
    /// - Toffoli → standard 15-gate decomposition using CNOT + H + T + T†
    fn decompose_gate(
        &self,
        gate: &dyn GateOp,
        spec: &HardwareSpec,
