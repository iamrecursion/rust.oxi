...
impl CircuitInterface {
    ...
    pub fn is_clifford_circuit(&self, circuit: &InterfaceCircuit) -> bool {
    ...
    fn estimate_execution_time(
        ...
        let base_time_per_gate = match backend {
            SimulationBackend::StateVector => 0.1,
    ...
    fn estimate_memory_requirements(
        ...
        match backend {
            ...
            SimulationBackend::Stabilizer => circuit.num_qubits * circuit.num_qubits * 2,
            SimulationBackend::Sparse => {
                circuit.gates.len() * 1000 * std::mem::size_of::<Complex64>()
            }
            SimulationBackend::TensorNetwork => {
                circuit.num_qubits * 64 * std::mem::size_of::<Complex64>()
            }
    ...
    pub fn unitary_matrix(&self) -> Result<Array2<Complex64>> {
        match &self.gate_type {
            ...
            InterfaceGateType::MultiControlledZ(num_controls) => {
                ...
                let target_state = (1 << total_qubits) - 1;
            ...
            InterfaceGateType::MultiControlledX(num_controls) => {
                ...
                let dim = 1 << total_qubits;
                let mut matrix = Array2::eye(dim);
                let control_pattern = (1 << *num_controls) - 1;
                let target_bit = 1 << num_controls;
                let state0 = control_pattern;
                let state1 = control_pattern | target_bit;
