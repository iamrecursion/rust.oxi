    ...
    pub fn create_initial_pattern(pattern_type: &str, dimensions: &[usize]) -> Array1<Complex64> {
        ...
        match pattern_type {
            ...
            "glider" if dimensions.len() == 2 => {
                ...
                if width >= 3 && height >= 3 {
                    let glider_positions = [width, 2 * width + 1, 2, width + 2, 2 * width + 2];
...
pub struct QCARule {
    ...
    pub neighborhood_pattern: Vec<(i32, i32, i32)>,
    /// Rule parameters
...
impl QuantumCellularAutomaton {
    ...
    fn evolve_margolus(&mut self) -> Result<()> {
        ...
        let (width, height) = (self.config.dimensions[0], self.config.dimensions[1]);
        let offset = self.stats.evolution_steps % 2;
    ...
    pub fn calculate_entanglement_entropy(&self) -> Result<f64> {
    ...
    pub fn calculate_local_observables(&self) -> Result<HashMap<String, f64>> {
    ...
    fn create_default_rules(
        ...
        if config.rule_type == QCARuleType::Global {
            let state_size = 1 << cell_mapping.total_cells.min(10);
    ...
    pub fn create_cnot_unitary() -> Array2<Complex64> {
    ...
    pub fn create_rotation_unitary(angle: f64) -> Array2<Complex64> {
    ...
    pub fn is_unitary(matrix: &Array2<Complex64>) -> bool {
