...
mod cluster_distance_tests {
    ...
    fn test_inter_cluster_distances() {
        ...
        let distances =
            inter_cluster_distances(&x, &labels, "manhattan").expect("Operation failed");
        assert!(!distances.is_empty());
    ...
    fn test_intra_cluster_distances() {
        ...
        assert!(*distances.get(&0).expect("Operation failed") < 1.0); // Cluster 0 is compact
        assert!(*distances.get(&1).expect("Operation failed") < 1.0); // Cluster 1 is also compact

        // Test cosine distance
        let distances = intra_cluster_distances(&x, &labels, "cosine").expect("Operation failed");
        assert_eq!(distances.len(), 2);

        // Cosine distance for similar vectors should be small
        assert!(*distances.get(&0).expect("Operation failed") < 0.1);
        assert!(*distances.get(&1).expect("Operation failed") < 0.1);
    }
    ...
    fn test_distance_ratio_index() {
        ...
        let good_ratio =
            distance_ratio_index(&well_separated, &labels, "euclidean").expect("Operation failed");
        let poor_ratio = distance_ratio_index(&poorly_separated, &labels, "euclidean")
            .expect("Operation failed");
    ...
    fn test_isolation_index() {
        ...
        let good_isolation =
            isolation_index(&well_isolated, &labels, "euclidean").expect("Operation failed");
        let poor_isolation =
            isolation_index(&poorly_isolated, &labels, "euclidean").expect("Operation failed");
