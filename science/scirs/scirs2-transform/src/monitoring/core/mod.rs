...
impl EnsembleAnomalyDetector {
    ...
    /// Detect ensemble anomalies by combining the *actual* results of the
    /// individual member detectors that ran for each metric.
    ///
    /// `detector_results` maps `metric_name -> (detector_name -> the
    /// AnomalyRecord that detector produced for this metric)`; a detector
    ...
    /// - `vote_fraction` = (sum of weights of flagging detectors) / (sum of
    ///   all configured weights). Must be `>= self.voting_threshold`.
    /// - `confidence` = a saturating `[0, 1)` transform of the mean raw
    ///   `anomaly_score` reported by the flagging detectors (their scores
    ///   live on different, unbounded scales -- z-score, IQR-distance-ratio,
    ///   etc. -- so this puts them on a common, comparable footing). Must
    ///   be `>= self.confidence_threshold`.
    ///
    ...
    pub fn detect_ensemble_anomalies(
        ...
        let total_weight: f64 = self.detector_weights.values().sum();
        if total_weight <= 0.0 {
            return Ok(ensemble_anomalies);
        }
        for (metric_name, &value) in metrics {
            let Some(per_detector) = detector_results.get(metric_name) else {
                continue;
            };
            if per_detector.is_empty() {
                continue;
            }
            let mut flagged_weight = 0.0;
            let mut score_sum = 0.0;
            for (detector_name, record) in per_detector {
                flagged_weight += self
                    .detector_weights
                    .get(detector_name)
                    .copied()
                    .unwrap_or(0.0);
                score_sum += record.anomaly_score;
            }
            let vote_fraction = flagged_weight / total_weight;
            if vote_fraction < self.voting_threshold {
                continue;
            }
            let mean_score = score_sum / per_detector.len() as f64;
            let confidence = 1.0 - (-mean_score.max(0.0)).exp();
            if confidence < self.confidence_threshold {
                continue;
            }
            let mut contributing: Vec<&str> = per_detector.keys().map(|s| s.as_str()).collect();
            contributing.sort_unstable();
            let severity = if vote_fraction >= 0.9 {
                AnomalySeverity::Critical
            } else if vote_fraction >= 0.75 {
                AnomalySeverity::High
            } else if vote_fraction >= 0.6 {
                AnomalySeverity::Medium
            } else {
                AnomalySeverity::Low
            };
            ensemble_anomalies.push(AnomalyRecord {
                timestamp,
                metric_name: metric_name.clone(),
                value,
                anomaly_score: vote_fraction,
                detection_method: "ensemble_weighted_vote".to_string(),
                severity,
                context: [
                    ("vote_fraction".to_string(), vote_fraction.to_string()),
                    ("confidence".to_string(), confidence.to_string()),
                    ("contributing_detectors".to_string(), contributing.join(",")),
                ]
                .into_iter()
                .collect(),
            });
...
impl AdvancedAnomalyDetector {
    ...
    pub fn detect_anomalies(
        ...
        let mut detector_results: HashMap<String, HashMap<String, AnomalyRecord>> = HashMap::new();
        let timestamp = current_timestamp();
        for (metric_name, &value) in metrics {
            if let Some(detector) = self.statistical_detectors.get_mut(metric_name) {
                if let Some(anomaly) = detector.detect_anomaly(value, metric_name, timestamp)? {
                    detector_results
                        ...
                        .or_default()
                        .insert("statistical".to_string(), anomaly.clone());
            ...
            if let Some(detector) = self.ml_detectors.get_mut(metric_name) {
                if let Some(anomaly) = detector.detect_anomaly(value, metric_name, timestamp)? {
                    detector_results
                        .entry(metric_name.clone())
                        .or_default()
                        .insert("ml".to_string(), anomaly.clone());
                    anomalies.push(anomaly);
                }
            }
            if let Some(detector) = self.time_series_detectors.get_mut(metric_name) {
                if let Some(anomaly) = detector.detect_anomaly(value, metric_name, timestamp)? {
                    detector_results
                        .entry(metric_name.clone())
                        .or_default()
                        .insert("time_series".to_string(), anomaly.clone());
                    anomalies.push(anomaly);
                }
            }
        }
        if let Some(ref ensemble) = self.ensemble_detector {
            let ensemble_anomalies =
                ensemble.detect_ensemble_anomalies(metrics, &detector_results, timestamp)?;
...
impl StatisticalDetector {
    ...
    pub fn new(z_score_threshold: f64, iqr_multiplier: f64, max_window_size: usize) -> Self {
        StatisticalDetector {
            z_score_threshold,
            iqr_multiplier,
            modified_z_threshold: z_score_threshold * 0.6745,
...
impl MLAnomalyDetector {
    ...
    pub fn detect_anomaly(
        ...
        let anomaly_score = self.isolation_forest_score(value)?;
        let threshold = 1.0 - self.isolation_forest_config.contamination;
        if anomaly_score > threshold {
            ...
            return Ok(Some(AnomalyRecord {
                ...
                context: [
                    ...
                    (
                        "n_trees".to_string(),
                        self.isolation_forest_config.n_trees.to_string(),
                    ),
                ]
                .iter()
                .cloned()
    ...
    /// Real Isolation Forest anomaly score for `value` against the current
    /// training window, following Liu, Ting & Zhou (2008): build
    /// `n_trees` isolation trees (each over an independent random
    /// sub-sample of up to `max_samples` points, height-limited to
    /// `ceil(log2(subsample_size))`), measure `value`'s average path
    /// length to isolation across all trees, and normalize it by `c(n)`
    /// (the expected path length of an unsuccessful BST search over `n`
    /// points) into the standard `2^(-E[h(x)] / c(n))` score in `[0, 1]`
    /// (values near 1 are anomalous, near 0.5 are typical).
    fn isolation_forest_score(&self, value: f64) -> Result<f64> {
        ...
        let sample_size = self.isolation_forest_config.max_samples.clamp(2, n);
        let max_depth = (sample_size as f64).log2().ceil().max(1.0) as usize;
        let mut rng = rng();
        let mut total_path_length = 0.0;
        for _ in 0..self.isolation_forest_config.n_trees.max(1) {
            let sample = subsample(&data, sample_size, &mut rng);
            let tree = IsolationNode::build(&sample, 0, max_depth, &mut rng);
            total_path_length += tree.path_length(value, 0);
        }
        let avg_path_length =
            total_path_length / self.isolation_forest_config.n_trees.max(1) as f64;
        let c = average_path_length_correction(sample_size);
        if c <= 0.0 {
            return Ok(0.5);
        }
        Ok(2f64.powf(-avg_path_length / c))
    }
}
/// A node of a single isolation tree over 1-D data.
enum IsolationNode {
    /// A leaf reached by exhausting the depth budget or isolating down to a
    /// single point; `size` is the number of points still in this node
    /// (>1 only when the depth limit was hit first), used for the standard
    /// `c(size)` path-length correction.
    Leaf { size: usize },
    /// An internal split: values `< split` go left, others go right.
    Split {
        split: f64,
...
impl IsolationNode {
    fn build(
        ...
        depth: usize,
        max_depth: usize,
        rng: &mut (impl scirs2_core::random::Rng + scirs2_core::random::RngExt),
    ) -> Self {
        ...
        let left: Vec<f64> = data.iter().copied().filter(|&v| v < split).collect();
        let right: Vec<f64> = data.iter().copied().filter(|&v| v >= split).collect();
    ...
    fn path_length(&self, value: f64, depth: usize) -> f64 {
        match self {
            ...
            IsolationNode::Split { split, left, right } => {
                if value < *split {
...
/// Search Tree built from `n` points (Liu, Ting & Zhou 2008, eq. 1),
/// used to normalize raw isolation-tree path lengths into a comparable
/// anomaly score. `H(i)` (harmonic number) is approximated by
/// `ln(i) + gamma` (Euler-Mascheroni), the same approximation used in the
/// original paper for non-trivial `n`.
fn average_path_length_correction(n: usize) -> f64 {
    if n <= 1 {
        ...
        let n = n as f64;
        let harmonic_n_minus_1 = (n - 1.0).ln() + EULER_MASCHERONI;
        2.0 * harmonic_n_minus_1 - (2.0 * (n - 1.0) / n)
    }
}
/// Draw a uniform random sub-sample of `sample_size` values from `data`
