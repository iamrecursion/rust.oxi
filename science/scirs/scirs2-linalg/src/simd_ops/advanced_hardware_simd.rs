...
impl AdvancedHardwareCapabilities {
    ...
    pub fn optimal_blocking_strategy(&self) -> BlockingStrategy {
        if self.has_amx_tile {
            BlockingStrategy::AMXTiling {
                tile_m: 16,
                tile_n: 64,
                tile_k: 64,
            }
        } else if self.has_avx512f {
            BlockingStrategy::AVX512 {
                block_m: 6,
                block_n: 16,
                block_k: 128,
            }
...
/// # Safety
///
/// - `a` must be a valid pointer to at least `m * lda` i8 elements.
/// - `b` must be a valid pointer to at least `k * ldb` i8 elements.
/// - `c` must be a valid mutable pointer to at least `m * ldc` i32 elements.
/// - All pointers must be properly aligned and non-null.
/// - This function uses ARM Neon SIMD intrinsics and must only be called on
///   aarch64 targets with the `neon` and `dotprod` features enabled.
    ...
    for i in (0..m).step_by(MR) {
        for j in (0..n).step_by(NR) {
            ...
            for l in (0..k_aligned).step_by(4) {
                ...
                // Dot product accumulation (using stable pairwise multiply-add)
                for ii in 0..MR.min(m - i) {
                    for jj in 0..NR.min(n - j) {
                        // Manual dot product: multiply pairs and accumulate to i32
                        let lo_a = vget_low_s8(a_vecs[ii]);
                        let lo_b = vget_low_s8(b_vecs[jj]);
                        let hi_a = vget_high_s8(a_vecs[ii]);
                        let hi_b = vget_high_s8(b_vecs[jj]);
                        let prod_lo = vmull_s8(lo_a, lo_b);
                        let prod_hi = vmull_s8(hi_a, hi_b);
                        let sum_lo = vpaddlq_s16(prod_lo);
                        let sum_hi = vpaddlq_s16(prod_hi);
                        c_acc[ii][jj] = vaddq_s32(c_acc[ii][jj], vaddq_s32(sum_lo, sum_hi));
                    }
                }
            }
...
impl AdaptiveSIMDDispatcher {
    ...
    pub fn adaptive_gemm_f32(
        ...
        let mut result: Array2<f32> = Array2::zeros((m, n));
    ...
    fn fallback_gemm(&self, a: &ArrayView2<f32>, b: &ArrayView2<f32>) -> LinalgResult<Array2<f32>> {
        let (m, k) = a.dim();
        let (_, n) = b.dim();
        let mut result = Array2::zeros((m, n));
...
impl CacheOptimizedOperations {
    pub fn new(capabilities: &AdvancedHardwareCapabilities) -> Self {
        Self {
            l1_blocksize: (capabilities.l1_cachesize / 8).next_power_of_two().min(256),
            l2_blocksize: (capabilities.l2_cachesize / 8)
                .next_power_of_two()
                .min(2048),
            l3_blocksize: (capabilities.l3_cachesize / 8)
                .next_power_of_two()
                .min(8192),
        }
    }
...
mod tests {
    ...
    fn test_adaptive_simd_dispatcher() {
        ...
        let result = dispatcher
            .adaptive_gemm_f32(&a.view(), &b.view())
    ...
    fn test_cache_optimized_transpose() {
        ...
        let result = cache_ops
            .cache_optimized_transpose_f32(&input.view())
            .expect("Operation failed");
